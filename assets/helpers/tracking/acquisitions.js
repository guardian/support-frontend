...
function isUsCampaignTest(referrerAcquisitionData: ReferrerAcquisitionData): boolean {
  if (referrerAcquisitionData.abTests) {
    return referrerAcquisitionData.abTests.includes((test: AcquisitionABTest) =>
      test.name === 'AcquisitionsEpicUsTopTickerRoundFive' || test.name === 'AcquisitionsBannerUsEoy');
  }
  return false;
}
