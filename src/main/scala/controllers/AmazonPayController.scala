...
class AmazonPayController(
  ...
  def executePayment: Action[AmazonPayRequest] =  CorsAction.async(circe.json[AmazonPayRequest]) {
    createRequest =>
  ...
  private def unmarshalNotification(notificationRequest: Request[AnyContent]) = {
    ...
    val notification = io.circe.parser.parse(bodyAsText.toString).flatMap {
      json =>
    ...
    EitherT.fromEither[Future](notification)
      .leftMap { error =>
  ...
  def notification(): Action[AnyContent] = CorsAction.async {
    notificationRequest =>
