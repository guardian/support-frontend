...
class SerialisationSpec extends FlatSpec with SerialisationTestHelpers with LazyLogging {
  ...
  "Circe" should "be able to decode a discount Promotion" in {
    val promotion = decode[Promotion](Fixtures.discountPromotion)
    assertDecodingSucceeded[Promotion](promotion)

    val discount = promotion.right.get.discount
    discount.isDefined shouldBe true
    discount.get.durationMonths shouldBe Some(months(12))
    discount.get.amount shouldBe 30

  }

  it should "be able to decode a free trial Promotion" in {
    val promotion = decode[Promotion](Fixtures.freeTrialPromotion)
    assertDecodingSucceeded[Promotion](promotion)

    val freeTrial = promotion.right.get.freeTrial
    freeTrial.isDefined shouldBe true
    freeTrial.get.duration shouldBe days(14)

    promotion.right.get.renewalOnly shouldBe false
  }

  it should "be able to decode a double benefit Promotion" in {
    val promotion = decode[Promotion](Fixtures.doublePromotion)
    assertDecodingSucceeded[Promotion](promotion)
