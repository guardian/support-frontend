...
class ContributorSpec extends FeatureSpec with Browser with GivenWhenThen with BeforeAndAfter with BeforeAndAfterAll {
  ...
  feature("Perform a one-off contribution") {
    ...
    scenario("One-off contribution with Stripe - GBP", SeleniumTag) {
      val testUser = new TestUser
      val country = "uk"
      val landingPageUk = ContributionsLanding(region = country)
      val expectedPayment = 50
      val oneOffContributionPage = OneOffContribution(testUser, expectedPayment)

      Given("that a test user goes to the UK contributions landing page")
      goTo(landingPageUk)
      assert(landingPageUk.pageHasLoaded)

      When("they click the one-time contribution toggle")
      landingPageUk.clickOneOff

      And("then click the contribute with debit/credit card button")
      landingPageUk.clickContribute

      Then("the contribute page should be loaded")
      assert(oneOffContributionPage.pageHasLoaded)

      And("the user should be warned to fill in all required fields")
      assert(oneOffContributionPage.errorMessageIsVisible)

      And("the PayPal payment button should be hidden")
      assert(!oneOffContributionPage.payPalButtonIsVisible)

      And("the pay with debit/credit card button should be hidden")
      assert(!oneOffContributionPage.stripeButtonIsVisible)

      Given("they fill in thier name")
      oneOffContributionPage.fillInName()

      When("and the email field")
      oneOffContributionPage.fillInEmail()

      Then("the warning message should be hidden")
      assert(!oneOffContributionPage.errorMessageIsVisible)

      And("the payment buttons should be visible")
      assert(oneOffContributionPage.stripeButtonIsVisible)
      assert(oneOffContributionPage.payPalButtonIsVisible)

      Given("they press the Stripe payment button")
      oneOffContributionPage.selectStripePayment()

      Then("the Stripe Checkout iFrame should display")
      assert(oneOffContributionPage.stripeCheckoutHasLoaded)

      Given("that the Stripe Checkout iFrame has the expected fields")
      oneOffContributionPage.switchToStripe()
      assert(oneOffContributionPage.stripeCheckoutHasCardNumberField)
      assert(oneOffContributionPage.stripeCheckoutHasExpiryField)
      assert(oneOffContributionPage.stripeCheckoutHasCvcField)
      assert(oneOffContributionPage.stripeCheckoutHasSubmitButton)

      When("they fill in valid credit card payment details")
      oneOffContributionPage.fillInCreditCardDetails()

      And("they click on the pay button")
      oneOffContributionPage.clickStripePayButton()

      Then("the one-off payment thankyou page should be displayed")
      OneOffContributionThankYou.focusOnDefaultFrame // ensure that we are looking at the main page, and not the Stripe iFrame that may have just closed
      assert(OneOffContributionThankYou.pageHasLoaded)
