...
object TestData {
  def buildSwitches(
    ...
    FeatureSwitches(Some(On), Some(On), Some(On), Some(On)),
  ...
  val validPaperRequest = CreateSupportWorkersRequest(
    ...
    productInformation = Some(ProductInformation("HomeDelivery", "EverydayPlus", None)),
  ...
  val validWeeklyRequest = CreateSupportWorkersRequest(
    ...
    productInformation = Some(ProductInformation("GuardianWeeklyDomestic", "Monthly", None)),
    firstDeliveryDate = Some(someDateNextMonth),
    paymentFields = stripePaymentFields,
    ophanIds = OphanIds(None, None),
    referrerAcquisitionData =
