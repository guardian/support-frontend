...
// constants
import { DigitalPack, fixDecimals, GuardianWeekly, Paper, sendTrackingEventsOnClick } from "helpers/productPrice/subscriptions";
...
import { Monthly, postIntroductorySixForSixBillingPeriod } from "helpers/productPrice/billingPeriods";
import { currencies, detect, fromCountryGroupId, glyph } from "helpers/internationalisation/currency";
import { digitalSubscriptionLanding, guardianWeeklyLanding, paperSubsUrl } from "helpers/urls/routes";
import PaperPackshot from "components/packshots/paper-packshot";
import { PriceCopy, PricingCopy } from "../subscriptionsLandingProps";
// types
...
const getSubscriptionCopy = (countryGroupId: CountryGroupId, pricingCopy: PricingCopy): ProductCopy[] => {
  if (countryGroupId === GBPCountries) {
    return [digital(countryGroupId, pricingCopy[DigitalPack], true), guardianWeekly(countryGroupId, pricingCopy[GuardianWeekly], false), paper(countryGroupId, pricingCopy[Paper], false), // Removing the link to the old paper+digital page during the June 21 Sale
    ...
    return [guardianWeekly(countryGroupId, pricingCopy[GuardianWeekly], true), digital(countryGroupId, pricingCopy[DigitalPack], false), premiumApp(countryGroupId)];
  }

  return [digital(countryGroupId, pricingCopy[DigitalPack], true), guardianWeekly(countryGroupId, pricingCopy[GuardianWeekly], false), premiumApp(countryGroupId)];
};
