...
import { SetCountryChangedAction } from "components/subscriptionCheckouts/address/addressFieldsStore";
...
function WeeklyCheckoutFormGifting(props: PropTypes) {
  ...
  return <Content modifierClasses={['your-details']}>
      <Layout aside={<Summary image={<GridImage gridId="checkoutPackshotWeeklyGifting" srcSizes={[696, 500]} sizes="(max-width: 740px) 50vw, 696" imgType="png" altText="" />} title="Guardian Weekly" description="" productPrice={price} billingPeriod={props.billingPeriod} changeSubscription={routes.guardianWeeklySubscriptionLandingGift} product={props.product} orderIsAGift />}>
        <Form onSubmit={ev => {
        ev.preventDefault();
        props.submitForm();
      }}>
          ...
          <FormSection title="Gift recipient's details" border="bottom">
            <Select css={marginBottom} id="title" label="Title" optional value={props.titleGiftRecipient} onChange={e => props.setTitleGift(e.target.value)}>
              <OptionForSelect>Select a title</OptionForSelect>
              {options(titles)}
            </Select>
            <PersonalDetailsGift firstNameGiftRecipient={props.firstNameGiftRecipient || ''} setFirstNameGift={props.setFirstNameGift} lastNameGiftRecipient={props.lastNameGiftRecipient || ''} setLastNameGift={props.setLastNameGift} emailGiftRecipient={props.emailGiftRecipient || ''} setEmailGift={props.setEmailGift} formErrors={((props.formErrors as any) as FormError<PersonalDetailsFormField>[])} />
          </FormSection>
          <FormSection title="Gift delivery date">
            <Rows>
              <RadioGroup id="startDate" error={firstError('startDate', props.formErrors)} legend="Gift delivery date">
                {days.map(day => {
          ...
          <FormSection title="Is the billing address the same as the recipient's address?">
            <Rows>
              <RadioGroup label="Is the billing address the same as the recipient's address?" hideLabel id="billingAddressIsSame" name="billingAddressIsSame" orienntation="vertical" error={firstError('billingAddressIsSame', props.formErrors)}>
                <Radio value="yes" label="Yes" name="billingAddressIsSame" checked={props.billingAddressIsSame === true} onChange={() => setBillingAddressIsSameHandler(true)} />
          ...
          {paymentMethods.length > 1 ? <FormSection title="How would you like to pay?">
              <PaymentMethodSelector country={props.billingCountry} paymentMethod={props.paymentMethod} setPaymentMethod={props.setPaymentMethod} validationError={firstError('paymentMethod', props.formErrors)} />
            </FormSection> : null}
