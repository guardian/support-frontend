...
import { detect, GBPCountries } from "helpers/internationalisation/countryGroup";
...
const getRegionalCopyFor = (region: CountryGroupId) => region === GBPCountries ? <span>Find clarity<br css={showOnMobile} /> with the Guardian&apos;s global magazine</span> : <span>Read The<br css={showOnMobile} /> Guardian in print</span>;
...
function WeeklyHero({
  ...
  return <PageTitle title={orderIsAGift ? 'Give the Guardian Weekly' : 'The Guardian Weekly'} theme="weekly">
      <CentredContainer>
        <Hero image={<GridImage gridId="weeklyCampaignHeroImg" srcSizes={[1000, 500, 140]} sizes="(max-width: 740px) 100%,
            (max-width: 1067px) 150%,
            500px" imgType="png" altText="A collection of Guardian Weekly magazines" />} roundelText={orderIsAGift ? null : roundelText}>
          <section css={weeklyHeroCopy}>
            {orderIsAGift ? <GiftHeadingAnimation /> : <h2 css={weeklyHeroTitle}>{title}</h2>}
            <p css={weeklyHeroParagraph}>
              {copy}
            </p>
            <ThemeProvider theme={buttonBrand}>
              <LinkButton onClick={sendTrackingEventsOnClick({
              id: 'options_cta_click',
              product: 'GuardianWeekly',
              componentType: 'ACQUISITIONS_BUTTON'
            })} priority="tertiary" iconSide="right" icon={<SvgArrowDownStraight />} href="#subscribe">
                See pricing options
              </LinkButton>
            </ThemeProvider>
          </section>
