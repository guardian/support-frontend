...
import { DigitalBillingPeriod } from "helpers/productPrice/billingPeriods";
...
import { finalPrice, getProductPrice } from "helpers/productPrice/productPrices";
...
import { FormField as PersonalDetailsFormField, FormField, FormFields } from "helpers/subscriptionsForms/formFields";
...
import { checkoutFormIsValid, validateCheckoutForm } from "helpers/subscriptionsForms/formValidation";
import { submitCheckoutForm, trackSubmitAttempt } from "helpers/subscriptionsForms/submit";
...
function mapStateToProps(state: CheckoutState) {
  return { ...getFormFields(state),
    ...
    amount: finalPrice(state.page.checkout.productPrices, state.common.internationalisation.countryId, state.page.checkout.billingPeriod).price,
...
function mapDispatchToProps() {
  return { ...formActionCreators,
    ...
    submitForm: () => (dispatch: Dispatch<Action>, getState: () => CheckoutState) => submitCheckoutForm(dispatch, getState()),
...
function DigitalCheckoutFormGift(props: PropTypes) {
  const productPrice = getProductPrice(props.productPrices, props.country, props.billingPeriod);
  ...
  return <Content>
      <CheckoutLayout aside={<OrderSummary image={<GridImage gridId={props.country === 'AU' ? 'editionsPackshotAusShort' : 'editionsPackshotShort'} srcSizes={[1000, 500]} sizes="(max-width: 740px) 50vw, 500" imgType="png" altText="" />} title="Digital Gift Subscription" description="Premium App + The Guardian Daily + Ad-free" productPrice={productPrice} billingPeriod={props.billingPeriod} changeSubscription={routes.digitalSubscriptionLandingGift} orderIsAGift />}>
        <Form onSubmit={ev => {
        ev.preventDefault();
        props.submitForm();
      }}>
          <FormSection title="Gift recipient's details">
            <PersonalDetailsDigitalGift firstNameGiftRecipient={props.firstNameGiftRecipient || ''} setFirstNameGift={props.setFirstNameGift} lastNameGiftRecipient={props.lastNameGiftRecipient || ''} setLastNameGift={props.setLastNameGift} emailGiftRecipient={props.emailGiftRecipient || ''} setEmailGift={props.setEmailGift} formErrors={((props.formErrors as any) as FormError<PersonalDetailsFormField>[])} />
          </FormSection>
          <FormSection title="Gift delivery date">
            <DatePickerWithError error={firstError('giftDeliveryDate', props.formErrors)} value={props.giftDeliveryDate} onChange={date => props.setDigitalGiftDeliveryDate(date)} />
          </FormSection>
          <FormSection title="Personalise your gift">
            <TextArea css={controlTextAreaResizing} id="gift-message" label="Gift message" maxlength={300} value={props.giftMessage} onChange={e => props.setGiftMessage(e.target.value)} optional />
          </FormSection>
          ...
          <FormSectionHiddenUntilSelected id="stripeForm" show={props.paymentMethod === Stripe} title="Your card details">
            <StripeProviderForCountry country={props.country} isTestUser={props.isTestUser} submitForm={props.submitForm} allErrors={[...props.formErrors, ...props.addressErrors]} setStripePaymentMethod={props.setStripePaymentMethod} validateForm={props.validateForm} buttonText="Pay for your gift" csrf={props.csrf} />
          </FormSectionHiddenUntilSelected>
          <FormSectionHiddenUntilSelected id="directDebitForm" show={props.paymentMethod === DirectDebit} title="Your account details">
            <DirectDebitForm buttonText="Pay for your gift" submitForm={props.submitForm} allErrors={[...props.formErrors, ...props.addressErrors]} submissionError={props.submissionError} submissionErrorHeading={submissionErrorHeading} />
          </FormSectionHiddenUntilSelected>
          {props.paymentMethod === PayPal ? <PayPalSubmitButton paymentMethod={props.paymentMethod} onPaymentAuthorised={props.onPaymentAuthorised} csrf={props.csrf} currencyId={props.currencyId} payPalHasLoaded={props.payPalHasLoaded} formIsValid={props.formIsValid} validateForm={props.validateForm} isTestUser={props.isTestUser} setupRecurringPayPalPayment={props.setupRecurringPayPalPayment} amount={props.amount} billingPeriod={props.billingPeriod} allErrors={[...props.formErrors, ...props.addressErrors]} /> : null}
