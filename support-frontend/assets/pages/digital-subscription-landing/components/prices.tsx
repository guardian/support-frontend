...
function Prices({
  ...
  return <section css={[pricesSection, cssOverrides]} id="subscribe">
      <h2 css={pricesHeadline}>{copy.title}</h2>
      <p css={pricesSubHeadline}>{copy.paragraph}</p>
      <FlexContainer cssOverrides={priceBoxes}>
        <PaymentSelection countryGroupId={countryGroupId} currencyId={currencyId} productPrices={productPrices} orderIsAGift={orderIsAGift} isUsingGuestCheckout={isUsingGuestCheckout} showPayPalButton={showPayPalButton} />
      </FlexContainer>
    </section>;
}
