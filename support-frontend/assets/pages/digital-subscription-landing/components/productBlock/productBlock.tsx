...
const Dropdown = ({
  ...
  showDropDown,
  product
}: DropdownPropTypes) => <div id={`product-details-${product}`} className={cx('product-block__dropdown--hide', {
...
const Button = ({
  ...
  handleClick,
  product
}: ButtonPropTypes) => <button aria-controls={`product-details-${product}`} aria-expanded={showDropDown ? 'true' : 'false'} onClick={handleClick} className={cx('product-block__button--hide', {
  ...
  render() {
    ...
    return <div className="hope-is-power__products">
        <section className="product-block__container hope-is-power--centered">
          ...
          <ProductCard title="UK Daily in The Guardian Editions app" subtitle="Each day&apos;s edition, in one simple, elegant app" image={dailyImage} second={false} />
          ...
          <Button showDropDown={state.showDropDownDaily} handleClick={() => this.handleClick('Daily')} product="daily" />
          <Plus />
          <ProductCard title="Premium access to The Guardian Live app" subtitle="Live news, as it happens" image={appImage} second />
          ...
          <Button showDropDown={state.showDropDownApp} handleClick={() => this.handleClick('App')} product="app" />
          <Plus />
          <AdFreeSection />
        </section>
      </div>;
