...
import { ProductSmall } from "components/product/productOptionSmall";
import "components/product/productOptionSmall";
import { BillingPeriod } from "helpers/productPrice/billingPeriods";
import { Annual, Monthly, Quarterly } from "helpers/productPrice/billingPeriods";
import { Option } from "helpers/types/option";
import "helpers/types/option";
import { ProductPrices, BillingPeriods, ProductPrice } from "helpers/productPrice/productPrices";
...
import { Product as ProductOptionType } from "components/product/productOption";
...
export const getProductOptions = (productPrices: ProductPrices, countryGroupId: CountryGroupId) => productPrices[countryGroups[countryGroupId].name].NoFulfilmentOptions.NoProductOptions;
export const getCurrencySymbol = (currencyId: IsoCurrency): string => currencies[currencyId].glyph;
export const getDisplayPrice = (currencyId: IsoCurrency, price: number) => getCurrencySymbol(currencyId) + fixDecimals(price);
export const getProductPrice = (productOptions: BillingPeriods, billingPeriod: BillingPeriod, currencyId: IsoCurrency): ProductPrice => productOptions[billingPeriod][currencyId];
...
const BILLING_PERIOD = {
  ...
  [Annual]: {
    ...
    salesCopy: (currencyId: IsoCurrency, displayPrice: number, promotionalPrice: Option<number>) => {
      ...
      return isNumeric(promotionalPrice) ? <span>
...
const getHeroCtaProps = (productPrices: ProductPrices, currencyId: IsoCurrency, countryGroupId: CountryGroupId): ProductSmall[] => {
  ...
  const createPaymentOption = (billingPeriod: BillingPeriod): ProductSmall => {
    ...
    const offerCopy = promotion && promotion.landingPage && promotion.landingPage.roundel ? promotion.landingPage.roundel : BILLING_PERIOD[digitalBillingPeriod].offer;
