...
import { fromCountryGroupName, International } from "helpers/internationalisation/countryGroup";
...
const shortTermDescription = postIntroductorySixForSixBillingPeriod === Quarterly ? 'Quarterly (13 weeks)' : 'Monthly (4 weeks)';
export default function WeeklyTerms(props: PromotionTermsPropTypes) {
  const includes6For6 = props.promotionTerms.productRatePlans.filter(ratePlan => ratePlan.includes('6 for 6')).length > 0;
...
function getCountryPrice(countryGroupName: CountryGroupName, prices: CountryGroupPrices) {
  const {
    currency
  } = fromCountryGroupName(countryGroupName);
...
function StandardCountryPrice(nameAndSaving: NameAndSaving, prices: CountryGroupPrices) {
  const {
    shortTermPrice,
    annualPrice
  } = getCountryPrice(nameAndSaving.name, prices);
  return <SansParagraph>
...
function StandardTerms(props: PromotionTermsPropTypes) {
  return <div>
      ...
      {orderedCountryGroupNames.map(nameAndSaving => StandardCountryPrice(nameAndSaving, props.productPrices[nameAndSaving.name]))}
    </div>;
}
...
function SixForSix(props: PromotionTermsPropTypes) {
  return <div>
      ...
      {orderedCountryGroupNames.map(nameAndSaving => SixForSixCountryPrice(nameAndSaving, props.productPrices[nameAndSaving.name]))}
    </div>;
}