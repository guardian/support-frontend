...
import { getContributionTypeFromSession, getContributionTypeFromUrl, getAmountFromUrl, getPaymentMethodFromSession, getValidPaymentMethods, getValidContributionTypesFromUrlOrElse } from "helpers/forms/checkouts";
...
import { checkIfEmailHasPassword, selectAmount, setGuestAccountCreationToken, updateOtherAmount, updateContributionTypeAndPaymentMethod, updatePaymentMethod, updateSelectedExistingPaymentMethod, updateUserFormData, setThankYouPageStage, loadPayPalExpressSdk, loadAmazonPaySdk } from "./contributionsLandingActions";
...
// ----- Functions ----- //
function getInitialPaymentMethod(contributionType: ContributionType, countryId: IsoCountry, countryGroupId: CountryGroupId, switches: Switches): PaymentMethod {
...
function selectInitialAmounts(state: State, dispatch: (...args: Array<any>) => any, selectedContributionType: ContributionType) {
  const {
    amounts
  } = state.common;
...
function selectInitialContributionTypeAndPaymentMethod(state: State, dispatch: (...args: Array<any>) => any, contributionTypes: ContributionTypes): ContributionType {
  const {
    countryId
  } = state.common.internationalisation;
  const {
