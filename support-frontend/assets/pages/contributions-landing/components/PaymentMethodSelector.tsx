...
import { updatePaymentMethod, updateSelectedExistingPaymentMethod, loadPayPalExpressSdk, loadAmazonPaySdk } from "../contributionsLandingActions";
import { isUsableExistingPaymentMethod } from "helpers/forms/existingPaymentMethods/existingPaymentMethods";
import { ExistingPaymentMethod, RecentlySignedInExistingPaymentMethod } from "helpers/forms/existingPaymentMethods/existingPaymentMethods";
...
import { getExistingPaymentMethodLabel, mapExistingPaymentMethodToPaymentMethod, subscriptionsToExplainerList, subscriptionToExplainerPart } from "helpers/forms/existingPaymentMethods/existingPaymentMethods";
...
type PropTypes = {
  ...
  updatePaymentMethod: (arg0: PaymentMethod) => (arg0: (...args: Array<any>) => any) => void;
  updateSelectedExistingPaymentMethod: (arg0: RecentlySignedInExistingPaymentMethod | typeof undefined) => Action;
  isTestUser: boolean;
  switches: Switches;
  payPalHasBegunLoading: boolean;
  amazonPayHasBegunLoading: boolean;
  loadPayPalExpressSdk: (contributionType: ContributionType) => (dispatch: (...args: Array<any>) => any) => void;
  loadAmazonPaySdk: (countryGroupId: CountryGroupId, isTestUser: boolean) => (dispatch: (...args: Array<any>) => any) => void;
...
const legend = <div className="secure-transaction">
...
const getFullExistingPaymentMethods = (props: PropTypes): RecentlySignedInExistingPaymentMethod[] => ((props.existingPaymentMethods || []).filter(isUsableExistingPaymentMethod) as any);
...
function withProps(props: PropTypes) {
  const paymentMethods: PaymentMethod[] = getValidPaymentMethods(props.contributionType, props.switches, props.countryId, props.countryGroupId);
  ...
  return <div className={classNameWithModifiers('form__radio-group', ['buttons', 'contribution-pay'])}>
      {legend}
      {paymentMethods.length ? <RadioGroup className="form__radio-group-list" error={showErrorMessage && 'Please select a payment method'}>
          {contributionTypeIsRecurring(props.contributionType) && !props.existingPaymentMethods && <div className="awaiting-existing-payment-options">
              <AnimatedDots appearance="medium" />
            </div>}
          {contributionTypeIsRecurring(props.contributionType) && fullExistingPaymentMethods.map((existingPaymentMethod: RecentlySignedInExistingPaymentMethod) => <>
              <Radio id={`paymentMethodSelector-existing${existingPaymentMethod.billingAccountId}`} name="paymentMethodSelector" type="radio" value={existingPaymentMethod} onChange={() => {
        ...
        }} checked={props.paymentMethod === mapExistingPaymentMethodToPaymentMethod(existingPaymentMethod) && props.existingPaymentMethod === existingPaymentMethod} arial-labelledby="payment_method" label={renderExistingLabelAndLogo(existingPaymentMethod)} cssOverrides={radioCss} />
              <div css={css`
                  font-size: small;
                  font-style: italic;
                  margin-left: 40px;
          ...
          {paymentMethods.map(paymentMethod => <Radio id={`paymentMethodSelector-${paymentMethod}`} name="paymentMethodSelector" type="radio" value={paymentMethod} onChange={() => {
        onPaymentMethodUpdate(paymentMethod, props);
      }} checked={props.paymentMethod === paymentMethod} label={renderLabelAndLogo(paymentMethod)} cssOverrides={radioCss} />)}
          {contributionTypeIsRecurring(props.contributionType) && props.existingPaymentMethods && props.existingPaymentMethods.length > 0 && fullExistingPaymentMethods.length === 0 && <li className="form__radio-group-item">
        ...
        </RadioGroup> : noPaymentMethodsErrorMessage}
    </div>;
}

function withoutProps() {
  return <fieldset className={classNameWithModifiers('form__radio-group', ['buttons', 'contribution-pay'])}>
      <legend className="form__legend">Payment method</legend>
      <ul className="form__radio-group-list">
        {[Stripe, PayPal, DirectDebit].map(paymentMethod => <li className="form__radio-group-item">
            <input id={`paymentMethodSelector-${paymentMethod}`} className="form__radio-group-input" name="paymentMethodSelector" type="radio" value={paymentMethod} />
