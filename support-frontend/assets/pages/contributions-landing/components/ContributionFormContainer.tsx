...
const defaultContributeCopy = <span>
...
const articleCountContributeCopy = currency => <span>
...
export const addArticleCountOptOutCookie = (): void => set(ARTICLE_COUNT_OPT_OUT_COOKIE.name, new Date().getTime().toString(), ARTICLE_COUNT_OPT_OUT_COOKIE.daysToLive);
export const removeArticleCountOptOutCookie = (): void => remove(ARTICLE_COUNT_OPT_OUT_COOKIE.name);
export const hasArticleCountOptOutCookie = (): boolean => !!get(ARTICLE_COUNT_OPT_OUT_COOKIE.name);
...
function withProps(props: PropTypes) {
  ...
  const campaignCopy = campaignSettings && campaignSettings.copy ? campaignSettings.copy(props.tickerGoalReached) : null;
  ...
  const countryGroupDetails = { ...defaultHeaderCopyAndContributeCopy,
    ...(campaignCopy || {})
  };
  ...
  if (props.campaignCodeParameter && !campaignSettings) {
    ...
    return <Redirect to={`/${countryGroups[props.countryGroupId].supportInternationalisationId}/contribute`} push={false} />;
  }
  ...
  return <div className="gu-content__content gu-content__content-contributions gu-content__content--flex">
      {!isArticleCountTest && showPreviousGiving && lastOneOffContribution && <ContributionFormBlurb headerCopy={<PreviousGivingHeaderCopy userName={props.userName} />} bodyCopy={<PreviousGivingBodyCopy lastOneOffContribution={lastOneOffContribution} />} />}

      {isArticleCountTest && numArticles && numArticles >= 5 ? <ContributionFormBlurb headerCopy={<ContributionsArticleCountWithOptOut numArticles={numArticles} isArticleCountOn={!hasOptedOut} isMobileOnly={false} onArticleCountOptOut={onArticleCountOptOut} onArticleCountOptIn={onArticleCountOptIn} defaultHeaderCopy={countryGroupDetails.headerCopy} userName={props.userName} />} bodyCopy={articleCountContributeCopy(glyph(props.currency))} /> : <ContributionFormBlurb headerCopy={countryGroupDetails.headerCopy} bodyCopy={countryGroupDetails.contributeCopy} />}

      <div className="gu-content__form">
        {isArticleCountTest && numArticles && <ContributionsArticleCountWithOptOut numArticles={numArticles} isMobileOnly isArticleCountOn={!hasOptedOut} onArticleCountOptOut={onArticleCountOptOut} onArticleCountOptIn={onArticleCountOptIn} userName={null} defaultHeaderCopy={null} />}

        <SecureTransactionIndicator modifierClasses={['top']} />

        {props.canShowTicker && campaignSettings && campaignSettings.tickerSettings ? <ContributionTicker {...campaignSettings.tickerSettings} onGoalReached={props.setTickerGoalReached} /> : null}
        {props.tickerGoalReached && campaignSettings && campaignSettings.tickerSettings && campaignSettings.goalReachedCopy ? campaignSettings.goalReachedCopy : <div>
            {countryGroupDetails.formMessage ? <div className="form-message">{countryGroupDetails.formMessage}</div> : null}

            <ContributionForm onPaymentAuthorisation={onPaymentAuthorisation} campaignSettings={campaignSettings} />
          </div>}
      </div>
      {campaignSettings && campaignSettings.extraComponent}
      <DirectDebitPopUpForm buttonText="Contribute with Direct Debit" onPaymentAuthorisation={onPaymentAuthorisation} />
    </div>;
}

function withoutProps() {
  return <div className="gu-content__content gu-content__content-contributions gu-content__content--flex">
      <ContributionFormBlurb headerCopy={defaultHeaderCopy} bodyCopy={defaultContributeCopy} />
