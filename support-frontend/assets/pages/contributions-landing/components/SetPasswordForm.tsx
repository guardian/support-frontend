...
import { setThankYouPageStage, setPasswordHasBeenSubmitted, setPasswordError, updatePassword } from "../contributionsLandingActions";
...
function onSubmit(props: PropTypes): (arg0: Event) => void {
  return event => {
    ...
    setPasswordGuest(props.password, props.guestAccountCreationToken, props.csrf).then(response => {
      if (response === true) {
...
function SetPasswordForm(props: PropTypes) {
  return <div className="set-password__form">
      <form onSubmit={onSubmit(props)} className={classNameWithModifiers('form', ['contribution'])} noValidate>
        <ContributionTextInput id="email" name="contribution-email" label="Email address" value={props.email} isValid={checkEmail(props.email)} pattern={emailRegexPattern} icon={<SvgEnvelope />} autoComplete="email" type="email" errorMessage="Please enter a valid email address" required disabled />
        <ContributionTextInput id="password" type="password" name="contribution-password" label="Set a password" icon={<SvgPasswordKey />} autoComplete="off" value={props.password} onInput={props.updatePassword} pattern={'^.{6,20}$'} isValid={props.password.length >= 6 && props.password.length <= 20} formHasBeenSubmitted={props.passwordHasBeenSubmitted} errorMessage="Please enter a password between 6 and 20 characters long" required />
        <TrackableButton appearance="secondary" modifierClasses={['create-account']} aria-label="Create a guardian account" type="submit" trackingEvent={() => {
        trackComponentLoad(`set-password-loaded-${props.contributionType}`);
      }}>
          Create a Guardian account
        </TrackableButton>
        <Button appearance="greyHollow" aria-label="No thank you" onClick={() => {
      ...
      {props.passwordError === true ? <div className="component-password-failure-message component-general-error-message">
