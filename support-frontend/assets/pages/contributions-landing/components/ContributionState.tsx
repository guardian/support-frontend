...
import { usStates, caStates, auStates } from "helpers/internationalisation/country";
import type { CountryGroupId, CountryGroup } from "helpers/internationalisation/countryGroup";
import { countryGroups, AUDCountries } from "helpers/internationalisation/countryGroup";
...
const renderState = (selectedState: StateProvince | null) => (state: {
  abbreviation: string;
  name: string;
}) => <option value={state.abbreviation} selected={selectedState === state.abbreviation}>&nbsp;&nbsp;{state.name}</option>;

const renderStatesField = (states: Record<string, string>, selectedState: StateProvince | null, onChange: ((arg0: Event) => void) | false, showError: boolean, label: string) => <div css={css`
        margin-top: 12px;
        margin-bottom: 4px;
        position: relative;
      `}>
    <label htmlFor="contributionState" css={css`
    ...
    {showError ? <InlineError cssOverrides={css`
            margin-bottom: 4px;
          `}>
    ...
    <div css={css`
          position: relative;
        `}>
      <select css={showError ? [selectCss, errorBorderCss] : selectCss} id="contributionState" onChange={onChange} required>
        <option value="">&nbsp;</option>
