...
const isLargeDonation = (amount: string, contributionType: ContributionType, paymentMethod: PaymentMethod): boolean => {
...
const ContributionThankYou = ({
  ...
  useEffect(() => {
    trackUserData(paymentMethod, contributionType, user.isSignedIn, !isNewAccount, isLargeDonation(amount, contributionType, paymentMethod));
  }, []);
  const signUpAction = {
  ...
  const marketingConsentAction = {
    component: <ContributionThankYouMarketingConsent email={email} csrf={csrf} />,
  ...
  const socialShareAction = {
    component: <ContributionThankYouSocialShare email={email} createReferralCodes={campaignSettings && campaignSettings.createReferralCodes} campaignCode={campaignSettings && campaignSettings.campaignCode} countryId={countryId} />,
    shouldShow: true
  };
  const ausMapAction = {
  ...
  const defaultActions = [signUpAction, signInAction, marketingConsentAction, supportReminderAction, surveyAction, socialShareAction];
  const ausActions = [signUpAction, signInAction, marketingConsentAction, supportReminderAction, surveyAction, ausMapAction, socialShareAction];
  const actions = countryId === 'AU' ? ausActions : defaultActions;
  const shownComponents = actions.filter(action => action.shouldShow).map(action => action.component);
  ...
  return <div css={container}>
      <div css={headerContainer}>
        <ContributionThankYouHeader name={name} showDirectDebitMessage={paymentMethod === DirectDebit} paymentMethod={paymentMethod} contributionType={contributionType} amount={amount} currency={currency} shouldShowLargeDonationMessage={isLargeDonation(amount, contributionType, paymentMethod)} />
      </div>
      ...
      <div css={buttonContainer}>
        <LinkButton href="https://www.theguardian.com" priority="tertiary" onClick={() => trackComponentClick(OPHAN_COMPONENT_ID_RETURN_TO_GUARDIAN)}>
