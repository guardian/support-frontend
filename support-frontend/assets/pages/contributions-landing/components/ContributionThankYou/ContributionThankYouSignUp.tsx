...
import { setPasswordError as setPasswordErrorAction, updatePassword as updatePasswordAction, type Action } from "../../contributionsLandingActions";
import { setPasswordGuest } from "helpers/paymentIntegrations/readerRevenueApis";
import styles from "./styles";
import { OPHAN_COMPONENT_ID_SIGN_UP, OPHAN_COMPONENT_ID_READ_MORE_SIGN_UP } from "./utils/ophan";
...
const checkPassword = (password: string) => password.length >= 6 && password.length <= 72;

const PASSWORD_ERROR_MESSAGE = 'Please enter a password between 6 and 72 characters long';
...
const ContributionThankYouSignUp = ({
  ...
  const actionHeader = <ActionHeader title={hasBeenCompleted ? 'You\u2019re almost there!' : 'Complete registration'} />;
      ...
      {hasBeenCompleted ? <p>
          ...
          <p>
            <span css={styles.hideAfterTablet}>
              ...
              {!isExpanded && <ButtonLink css={bodyText} priority="secondary" onClick={onReadMoreClick}>
                  Read more
                </ButtonLink>}
            </span>
          ...
          <div css={styles.hideBeforeTablet}>
            <div css={expandableContainer}>
              <BulletPointedList items={['Remove unnecessary messages asking you for financial support', 'Let you easily manage your recurring contributions, subscriptions and newsletters in one place']} />
            </div>
          </div>
          <div>
            <form onSubmit={onSubmit} css={form} noValidate>
              <div>
                <TextInput value={email} label="Email address" supporting="example@domain.com" disabled />
              </div>
              <div>
                <TextInput label="Set a password" supporting="Between 6 and 72 characters" value={password} onChange={updatePassword} error={passwordError ? PASSWORD_ERROR_MESSAGE : ''} required type="password" />
              </div>
              <div css={buttonContainer}>
                <Button type="submit" priority="primary" size="default" icon={<SvgArrowRightStraight />} iconSide="right" nudgeIcon>
                  Register
                </Button>
              </div>
            </form>
...
export default connect(mapStateToProps, mapDispatchToProps)(ContributionThankYouSignUp);