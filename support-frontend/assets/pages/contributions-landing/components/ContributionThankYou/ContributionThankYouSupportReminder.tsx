...
const getDefaultReminderChoices = (): ReminderChoice[] => [getRecurringReminderChoice(), getOneOffReminderChoiceWithDefaultLabel(3), getOneOffReminderChoiceWithDefaultLabel(6), getOneOffReminderChoiceWithDefaultLabel(9)];
...
const ContributionThankYouSupportReminder = ({
  ...
  const actionHeader = <ActionHeader title={hasBeenCompleted ? 'Your support reminder is set' : 'Set a support reminder'} />;
      ...
      {hasBeenCompleted ? <p>
          ...
          <form css={form}>
            <RadioGroup name="reminder" label="I'd like to be reminded in:">
              {reminderChoices.map((choice, index) => <Radio value={index} label={choice.label} checked={selectedChoiceIndex === index} onChange={() => setSelectedChoiceIndex(index)} />)}
            </RadioGroup>
          </form>
          <div css={buttonContainer}>
            <Button onClick={onSubmit} priority="primary" size="default" icon={<SvgArrowRightStraight />} iconSide="right" nudgeIcon>
              Set my reminder
            </Button>
          </div>
          <p css={privacyText}>
            ...
            visit our{' '}
            <Link css={privacyTextLink} href={privacyLink} target="_blank" rel="noopener noreferrer" priority="secondary">
              Privacy Policy
            </Link>
            .
          </p>
