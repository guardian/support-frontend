...
function mapDispatchToProps(dispatch: Dispatch<Action>) {
  return {
    subscribeToNewsLetter: (email: string, csrf: Csrf) => {
      sendMarketingPreferencesToIdentity(true, email, dispatch, csrf, 'MARKETING_CONSENT');
    }
  };
}
...
const ContributionThankYouMarketingConsent = ({
  ...
  const actionHeader = <ActionHeader title={hasBeenCompleted ? 'You\'re signed up' : 'Hear from our newsroom'} />;
      ...
      {hasBeenCompleted ? <p>
          ...
          <div css={checkboxContainer}>
            <CheckboxGroup error={errorMessage}>
              <Checkbox checked={hasConsented} onChange={() => setHasConsented(!hasConsented)} supporting="Stay up-to-date with our latest offers and the aims of the organisation, as well as the ways to enjoy and support our journalism." />
            </CheckboxGroup>
          </div>
