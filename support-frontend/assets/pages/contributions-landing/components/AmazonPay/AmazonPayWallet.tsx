...
import { AmazonPayData, State } from "pages/contributions-landing/contributionsLandingReducer";
import { Action } from "pages/contributions-landing/contributionsLandingActions";
import { setAmazonPayWalletIsStale, setAmazonPayOrderReferenceId, setAmazonPayPaymentSelected, setAmazonPayBillingAgreementId, setAmazonPayBillingAgreementConsentStatus } from "pages/contributions-landing/contributionsLandingActions";
...
const mapDispatchToProps = (dispatch: (...args: Array<any>) => any) => ({
  ...
  setAmazonPayOrderReferenceId: (orderReferenceId: string) => dispatch(setAmazonPayOrderReferenceId(orderReferenceId)),
  setAmazonPayPaymentSelected: (paymentSelected: boolean) => dispatch(setAmazonPayPaymentSelected(paymentSelected)),
  setAmazonPayBillingAgreementId: (amazonBillingAgreementId: string) => dispatch(setAmazonPayBillingAgreementId(amazonBillingAgreementId)),
  setAmazonPayBillingAgreementConsentStatus: (amazonPayBillingAgreementConsentStatus: boolean) => dispatch(setAmazonPayBillingAgreementConsentStatus(amazonPayBillingAgreementConsentStatus))
});

const getSellerId = (isTestUser: boolean): string => isTestUser ? window.guardian.amazonPaySellerId.uat : window.guardian.amazonPaySellerId.default;

const AmazonPayWalletComponent = (props: PropTypes) => {
  const createWalletWidget = (amazonPaymentsObject: Record<string, any>): void => {
    ...
    if (props.contributionType !== 'ONE_OFF') {
      new amazonPaymentsObject.Widgets.Wallet({ ...baseWalletConfig,
  ...
  const createConsentWidget = (amazonPaymentsObject: Record<string, any>, amazonBillingAgreementId: string): void => {
    const consentConfig = {
      ...
      onReady: billingAgreementConsentStatus => {
        const amazonBillingAgreementConsentStatus = billingAgreementConsentStatus.getConsentStatus() === 'true';
  ...
  }, [props.amazonPayData.amazonPayLibrary.amazonPaymentsObject, props.amazonPayData.walletIsStale]);
  useEffect(() => {
    const {
      amazonPaymentsObject
    } = props.amazonPayData.amazonPayLibrary;
    ...
    if (amazonPaymentsObject && props.amazonPayData.amazonBillingAgreementId && props.contributionType !== 'ONE_OFF') {
      createConsentWidget(amazonPaymentsObject, props.amazonPayData.amazonBillingAgreementId);
    }
  }, [props.amazonPayData.amazonPayLibrary.amazonPaymentsObject, props.amazonPayData.amazonBillingAgreementId, props.contributionType]);
  const {
    amazonLoginObject,
    amazonPaymentsObject
  } = props.amazonPayData.amazonPayLibrary;
  ...
  if (amazonLoginObject && amazonPaymentsObject) {
    ...
    return <div>
        {props.checkoutFormHasBeenSubmitted && !props.amazonPayData.paymentSelected && <InlineError>Please select a payment method</InlineError>}
        <div className="walletWidgetDiv" id="WalletWidgetDiv" />

        {props.contributionType !== 'ONE_OFF' && <div>
            {props.checkoutFormHasBeenSubmitted && !props.amazonPayData.amazonBillingAgreementConsentStatus && <InlineError>Please tick the box to agree to a recurring payment</InlineError>}
