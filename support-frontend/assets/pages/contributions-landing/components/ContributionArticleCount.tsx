...
// -- Functions -- //
const OPHAN_COMPONENT_ID_ARTICLE_COUNT_OPT_OUT_OPEN = 'contributions-epic-article-count-open';
const OPHAN_COMPONENT_ID_ARTICLE_COUNT_OPT_OUT_CLOSE = 'contributions-epic-article-count-close';
const OPHAN_COMPONENT_ID_ARTICLE_COUNT_STAY_IN = 'contributions-epic-article-count-stay-on';
...
export const ArticleCountWithToggle = ({
  ...
  if (isArticleCountOn && numArticles >= 5) {
    return <div css={articleCountHeaderContainerStyles}>
        <div css={articleCountAboveContainerStyles}>
          <ArticleCountHeaderCopy numArticles={numArticles} userName={userName} />
        </div>
        <div css={articleCountWrapperStyles}>
          <div css={articleCountTextStyles}>Article count</div>
          <ButtonLink priority="secondary" onClick={onToggleClick} cssOverrides={articleCountCtaStyles}>
            on
          </ButtonLink>
        </div>
      </div>;
...
export const ContributionsArticleCountWithOptOut = ({
  ...
  const onToggleClick = () => {
    setIsOpen(!isOpen);
    trackComponentClick(isOpen ? OPHAN_COMPONENT_ID_ARTICLE_COUNT_OPT_OUT_CLOSE : OPHAN_COMPONENT_ID_ARTICLE_COUNT_OPT_OUT_OPEN);
  };
  ...
  return <div css={isMobileOnly ? topContainerMobile : topContainerDesktop}>
      <ArticleCountWithToggle isArticleCountOn={isArticleCountOn} numArticles={numArticles} onToggleClick={onToggleClick} userName={userName} defaultHeaderCopy={defaultHeaderCopy} />

      {isOpen && <div css={articleCountDescriptionTopContainerStyles}>
          <div css={caretStyles} />
          <div css={articleCountDescriptionContainer}>
            {isArticleCountOn ? <>
                ...
                <div css={articleCountCtasContainerStyles}>
                  <Button priority="primary" size="xsmall" cssOverrides={articleCountDefaultCtaStyles} onClick={onStayInClick}>
                    Yes, thats OK
                  </Button>
                  <Button priority="tertiary" size="xsmall" cssOverrides={articleCountOptOutCtaStyles} onClick={onOptOutClick}>
                    No, opt me out
                  </Button>
                </div>
              </> : <>
                ...
                <div css={articleCountCtasContainerStyles}>
                  <Button priority="primary" size="xsmall" cssOverrides={articleCountOptInCtaStyles} onClick={onOptInClick}>
                    Yes, opt me in
                  </Button>
                  <Button priority="tertiary" size="xsmall" cssOverrides={articleCountOptOutCtaStyles} onClick={onStayOutClick}>
                    No, thank you
                  </Button>
                </div>
              </>}
          ...
          <div css={trackingSettingsContainerStyles}>
            ...
            <ButtonLink priority="secondary" cssOverrides={privacySettingsLinkStyles} onClick={openCmp}>
              Privacy Settings
            </ButtonLink>
          </div>
        </div>}
