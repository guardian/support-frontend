...
import { State, Stripe3DSResult } from "pages/contributions-landing/contributionsLandingReducer";
import { Stripe } from "helpers/forms/paymentMethods";
import { PaymentResult } from "helpers/forms/paymentIntegrations/readerRevenueApis";
...
import { onThirdPartyPaymentAuthorised, paymentFailure, paymentWaiting as setPaymentWaiting, setCreateStripePaymentMethod, setHandleStripe3DS, setStripeCardFormComplete, setStripeRecurringRecaptchaVerified, setStripeSetupIntentClientSecret } from "pages/contributions-landing/contributionsLandingActions";
...
type PropTypes = {
  ...
  setCreateStripePaymentMethod: (arg0: (clientSecret: string | null) => void) => Action;
...
const mapDispatchToProps = (dispatch: (...args: Array<any>) => any) => ({
  onPaymentAuthorised: (paymentMethodId: string) => dispatch(onThirdPartyPaymentAuthorised({
    paymentMethod: Stripe,
    stripePaymentMethod: 'StripeCheckout',
    paymentMethodId
  })),
  paymentFailure: (paymentError: ErrorReason) => dispatch(paymentFailure(paymentError)),
  setCreateStripePaymentMethod: (createStripePaymentMethod: (clientSecret: string | null) => void) => dispatch(setCreateStripePaymentMethod(createStripePaymentMethod)),
  ...
  setStripeCardFormComplete: (isComplete: boolean) => dispatch(setStripeCardFormComplete(isComplete)),
...
const errorMessageFromState = (state: CardFieldState): string | null => state.name === 'Error' ? state.errorMessage : null;
...
const CardForm = (props: PropTypes) => {
  ...
  const [fieldStates, setFieldStates] = useState<Record<CardFieldName, CardFieldState>>({
    CardNumber: {
      name: 'Incomplete'
    },
    Expiry: {
  ...
  const onChange = (fieldName: CardFieldName) => update => {
    ...
    setFieldStates(prevData => ({ ...prevData,
  ...
  const setupRecurringRecaptchaCallback = () => {
    ...
    window.grecaptcha.render('robot_checkbox', {
      sitekey: props.isTestUser ? window.guardian.v2recaptchaPublicKey.uat : window.guardian.v2recaptchaPublicKey.default,
      callback: token => {
        trackComponentLoad('contributions-recaptcha-client-token-received');
        props.setStripeRecurringRecaptchaVerified(true);
        fetchJson(routes.stripeSetupIntentRecaptcha, requestOptions({
          token,
          stripePublicKey: props.stripeKey,
          isTestUser: props.isTestUser
        }, 'same-origin', 'POST', props.csrf)).then(json => {
  ...
  useEffect(() => {
    const formIsComplete = fieldStates.CardNumber.name === 'Complete' && fieldStates.Expiry.name === 'Complete' && fieldStates.CVC.name === 'Complete' && isZipCodeFieldValid();
   ...
   * Rendering
   */
  const fieldError: string | null | undefined = errorMessageFromState(fieldStates.CardNumber) || errorMessageFromState(fieldStates.Expiry) || errorMessageFromState(fieldStates.CVC);
  const showZipCodeError = props.checkoutFormHasBeenSubmitted && !isZipCodeFieldValid();

  const incompleteMessage = (): string | null | undefined => {
    if (props.checkoutFormHasBeenSubmitted && (fieldStates.CardNumber.name === 'Incomplete' || fieldStates.Expiry.name === 'Incomplete' || fieldStates.CVC.name === 'Incomplete')) {
  ...
  const errorMessage: string | null | undefined = fieldError || incompleteMessage();
  ...
  const recaptchaVerified = props.contributionType === 'ONE_OFF' ? props.oneOffRecaptchaToken : props.recurringRecaptchaVerified;
  return <div>
      <legend className="form__legend"><h3>Your card details</h3></legend>
      {errorMessage ? <InlineError> {errorMessage} </InlineError> : null}

      <StripeCardFormField label={<>
            <label htmlFor="stripeCardNumberElement">
              Card number
            </label>
            {showCards(props.country)}
          </>} input={<CardNumberElement id="stripeCardNumberElement" options={{
      style: fieldStyle
    }} onChange={onChange('CardNumber')} onFocus={() => setCurrentlySelected('CardNumber')} onBlur={setCurrentlySelected(null)} />} error={fieldStates.CardNumber.name === 'Error'} focus={currentlySelected === 'CardNumber'} />

      <div className="ds-stripe-card-input__expiry-security-container">
        <div className="ds-stripe-card-input__expiry">
          <StripeCardFormField label={<label htmlFor="stripeCardExpiryElement">
                Expiry date
              </label>} hint={<div className="ds-stripe-card-input__expiry-hint">
                MM / YY
              </div>} input={<CardExpiryElement id="stripeCardExpiryElement" options={{
          style: fieldStyle
        }} placeholder="" onChange={onChange('Expiry')} onFocus={() => setCurrentlySelected('Expiry')} onBlur={setCurrentlySelected(null)} />} error={fieldStates.Expiry.name === 'Error'} focus={currentlySelected === 'Expiry'} />
        </div>

        <div className="ds-stripe-card-input__security-code">
          <StripeCardFormField label={<label htmlFor="stripeCardCVCElement">
                Security code
              </label>} hint={<div className="ds-stripe-card-input__security-code-hint">
              ...
              </div>} input={<CardCvcElement id="stripeCardCVCElement" options={{
          style: fieldStyle
        }} placeholder="" onChange={onChange('CVC')} onFocus={() => setCurrentlySelected('CVC')} onBlur={setCurrentlySelected(null)} />} error={fieldStates.CVC.name === 'Error'} focus={currentlySelected === 'CVC'} />
        </div>

      </div>

      {showZipCodeField && <div css={zipCodeContainerStyles}>
          <TextInput id="contributionZipCode" name="contribution-zip-code" label="ZIP code" value={zipCode} onChange={updateZipCode} error={showZipCodeError ? 'Please enter a valid ZIP code' : null} />
        </div>}

      {window.guardian.recaptchaEnabled ? <div className="ds-security-check">
          <div className="ds-security-check__label">
            <label htmlFor="robot_checkbox">
              Security check
            </label>
          </div>
          {props.checkoutFormHasBeenSubmitted && !recaptchaVerified ? renderVerificationCopy(props.countryGroupId, props.contributionType) : null}
          <Recaptcha />
        </div> : null}
    </div>;
};
