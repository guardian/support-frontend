...
const StripePaymentRequestButtonContainer = (props: PropTypes) => {
  ...
  const [prbObjects, setPrbObjects] = useState<Record<StripeAccount, Record<string, any> | null>>({
    ONE_OFF: null,
    REGULAR: null
  });
  ...
  if (showStripePaymentRequestButton && stripeObjects[stripeAccount]) {
    ...
    return <div className="stripe-payment-request-button" key={stripeAccount}>
        <Elements stripe={stripeObjects[stripeAccount]} options={elementsOptions}>
          <StripePaymentRequestButton stripeAccount={stripeAccount} amount={amount} stripeKey={stripeKey} paymentRequestObject={prbObjects[stripeAccount]} setPaymentRequestObject={prbObject => setPrbObjects({ ...prbObjects,
