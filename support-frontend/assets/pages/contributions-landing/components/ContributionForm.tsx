// ----- Imports ----- //
import { ThirdPartyPaymentLibraries } from "helpers/contributions";
...
import { ContributionAmounts, OtherAmounts, SelectedAmounts } from "helpers/contributions";
...
import { paymentWaiting, setCheckoutFormHasBeenSubmitted, createOneOffPayPalPayment, selectAmount, setSepaIban, setSepaAccountHolderName } from "pages/contributions-landing/contributionsLandingActions";
...
import { Sepa, DirectDebit, ExistingCard, ExistingDirectDebit, AmazonPay } from "helpers/forms/paymentMethods";
...
import { SepaData } from "pages/contributions-landing/contributionsLandingReducer";
...
type PropTypes = {
  ...
  setUseLocalCurrency: (arg0: boolean, arg1: LocalCurrencyCountry | null | undefined, arg2: number | null | undefined) => void;
...
// i.e it is null.
const getCheckoutFormValue = (formValue: string | null, userValue: string | null): string | null => formValue === null ? userValue : formValue;
...
const mapDispatchToProps = (dispatch: (...args: Array<any>) => any) => ({
  ...
  setUseLocalCurrency: (useLocalCurrency, localCurrencyCountry, defaultOneOffAmount) => {
    ...
    dispatch(selectAmount(useLocalCurrency ? localCurrencyCountry.amounts.ONE_OFF.defaultAmount : defaultOneOffAmount, 'ONE_OFF'));
  },
  setSepaIban: iban => {
...
const formHandlersForRecurring = {
  PayPal: () => {// we don't get an onSubmit event for PayPal recurring, so there
...
function withProps(props: PropTypes) {
  ...
  return <form onSubmit={onSubmit(props)} className={classNameWithModifiers(baseClass, classModifiers)} noValidate>
      <h2 className="hidden-heading">Make a contribution</h2>
      <div className="contributions-form-selectors">
        <ContributionTypeTabs />
        <ContributionAmount />
        {props.localCurrencyCountry && props.contributionType === 'ONE_OFF' && <CheckboxGroup cssOverrides="margin-top:16px;">
            <Checkbox label={`View in local currency (${props.localCurrencyCountry.currency})`} defaultChecked={props.useLocalCurrency} onChange={toggleUseLocalCurrency} />
          </CheckboxGroup>}
      </div>
      <StripePaymentRequestButtonContainer currency={props.currency} contributionType={props.contributionType} isTestUser={props.isTestUser} country={props.country} otherAmounts={props.otherAmounts} selectedAmounts={props.selectedAmounts} />
      <div className={classNameWithModifiers('form', ['content'])}>
        <ContributionFormFields />
        <PaymentMethodSelector />

        {props.paymentMethod === Sepa && <>
            <SepaForm iban={props.sepaData.iban} accountHolderName={props.sepaData.accountHolderName} updateIban={props.setSepaIban} updateAccountHolderName={props.setSepaAccountHolderName} checkoutFormHasBeenSubmitted={props.checkoutFormHasBeenSubmitted} />
            <SepaTerms />
          </>}

        <StripeCardFormContainer currency={props.currency} contributionType={props.contributionType} paymentMethod={props.paymentMethod} isTestUser={props.isTestUser} country={props.country} />
      ...
      <TermsPrivacy countryGroupId={props.countryGroupId} contributionType={props.contributionType} campaignSettings={props.campaignSettings} referrerSource={props.referrerSource} />
      {props.isWaiting ? <ProgressMessage message={['Processing transaction', 'Please wait']} /> : null}
    </form>;
}

function withoutProps() {
  return <form className={classNameWithModifiers('form', ['contribution'])}>
      <div>
