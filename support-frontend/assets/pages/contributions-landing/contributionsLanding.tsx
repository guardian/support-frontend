...
import { type CountryGroupId, countryGroups, detect } from "helpers/internationalisation/countryGroup";
...
const setOneOffContributionCookie = () => {
  setCookie(ONE_OFF_CONTRIBUTION_COOKIE, currentTimeInEpochMilliseconds.toString(), cookieDaysToLive);
};

const campaignSettings = getCampaignSettings();
const cssModifiers = campaignSettings && campaignSettings.cssModifiers ? campaignSettings.cssModifiers : [];
const backgroundImageSrc = campaignSettings && campaignSettings.backgroundImage ? campaignSettings.backgroundImage : null;
FocusStyleManager.onlyShowFocusOnTabs(); // https://www.theguardian.design/2a1e5182b/p/6691bb-accessibility

const contributionsLandingPage = (campaignCodeParameter: string | null | undefined) => <Page classModifiers={['new-template', 'contribution-form', ...cssModifiers]} header={<RoundelHeader selectedCountryGroup={selectedCountryGroup} />} footer={<ContributionsFooter />} backgroundImageSrc={backgroundImageSrc}>
    <ContributionFormContainer thankYouRoute={`/${countryGroups[countryGroupId].supportInternationalisationId}/thankyou`} campaignCodeParameter={campaignCodeParameter} />
  </Page>;

const router = <BrowserRouter>
    <Provider store={store}>
      <div>
        <Route exact path="/:countryId(uk|us|au|eu|int|nz|ca)/contribute/" render={() => contributionsLandingPage()} />
        <Route exact path="/:countryId(uk|us|au|eu|int|nz|ca)/contribute/:campaignCode" render={props => contributionsLandingPage(props.match.params.campaignCode)} />
        <Route exact path="/:countryId(uk|us|au|eu|int|nz|ca)/thankyou" render={props => {
