...
const setGuestAccountCreationToken = (
	guestAccountCreationToken: string,
): Action => ({
	type: 'SET_GUEST_ACCOUNT_CREATION_TOKEN',
...
const makeCreateStripePaymentIntentRequest =
	(
		...
		setGuestToken: (arg0: string) => void,
		setThankYouPage: (arg0: ThankYouPageStage) => void,
		handleStripe3DS: (clientSecret: string) => Promise<Stripe3DSResult>,
		paymentAuthorisation: PaymentAuthorisation,
	) =>
...
const paymentAuthorisationHandlers: PaymentMatrix<
	(
		arg0: Dispatch<Action>,
		arg1: State,
		arg2: PaymentAuthorisation,
	) => Promise<PaymentResult>
> = {
	ONE_OFF: {
		PayPal: () => {
			...
			if (paymentAuthorisation.paymentMethod === Stripe) {
				if (paymentAuthorisation.paymentMethodId) {
					...
					if (handle3DS) {
						...
						return dispatch(
							makeCreateStripePaymentIntentRequest(
								stripeData,
								(token: string) =>
									dispatch(setGuestAccountCreationToken(token)),
								(thankYouPageStage: ThankYouPageStage) =>
									dispatch(setThankYouPageStage(thankYouPageStage)),
								handle3DS,
								paymentAuthorisation,
