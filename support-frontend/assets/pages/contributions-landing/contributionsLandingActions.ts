...
import { type ContributionType, getAmount, logInvalidCombination, type PaymentMatrix } from "helpers/contributions";
...
import type { RegularPaymentRequest, StripePaymentIntentAuthorisation, StripePaymentMethod } from "helpers/paymentIntegrations/readerRevenueApis";
import { type AmazonPayAuthorisation, type PaymentAuthorisation, type PaymentResult, type StripePaymentRequestButtonMethod, postRegularPaymentRequest, regularPaymentFieldsFromAuthorisation } from "helpers/paymentIntegrations/readerRevenueApis";
...
import { type CreatePaypalPaymentData, type CreatePayPalPaymentResponse, postOneOffPayPalCreatePaymentRequest, processStripePaymentIntentRequest, postOneOffAmazonPayExecutePaymentRequest } from "helpers/paymentIntegrations/oneOffContributions";
...
export type Action = {
  ...
  type: "SET_THANK_YOU_PAGE_STAGE";
  thankYouPageStage: ThankYouPageStage;
} | {
  type: "SET_STRIPE_PAYMENT_REQUEST_OBJECT";
  stripePaymentRequestObject: Record<string, any>;
...
const setStripePaymentRequestObject = (stripePaymentRequestObject: Record<string, any>, stripeAccount: StripeAccount): Action => ({
  type: 'SET_STRIPE_PAYMENT_REQUEST_OBJECT',
...
const setPasswordHasBeenSubmitted = (): Action => ({
  type: 'SET_PASSWORD_HAS_BEEN_SUBMITTED'
});

const setPasswordError = (passwordError: boolean): Action => ({
  type: 'SET_PASSWORD_ERROR',
  passwordError
});
...
const checkIfEmailHasPassword = (email: string) => (dispatch: (...args: Array<any>) => any, getState: () => State): void => {
...
const makeCreateStripePaymentIntentRequest = (data: CreateStripePaymentIntentRequest, setGuestToken: (arg0: string) => void, setThankYouPage: (arg0: ThankYouPageStage) => void, handleStripe3DS: (clientSecret: string) => Promise<Stripe3DSResult>, paymentAuthorisation: PaymentAuthorisation) => (dispatch: Dispatch<Action>): Promise<PaymentResult> => dispatch(onPaymentResult(processStripePaymentIntentRequest(data, handleStripe3DS)(setGuestToken, setThankYouPage), paymentAuthorisation));

const executeAmazonPayOneOffPayment = (data: AmazonPayData, setGuestToken: (arg0: string) => void, setThankYouPage: (arg0: ThankYouPageStage) => void, paymentAuthorisation: PaymentAuthorisation) => (dispatch: Dispatch<Action>): Promise<PaymentResult> => dispatch(onPaymentResult(postOneOffAmazonPayExecutePaymentRequest(data)(setGuestToken, setThankYouPage), paymentAuthorisation));
...
function recurringPaymentAuthorisationHandler(dispatch: Dispatch<Action>, state: State, paymentAuthorisation: PaymentAuthorisation): Promise<PaymentResult> {
  ...
  return dispatch(onPaymentResult(postRegularPaymentRequest(routes.recurringContribCreate, request, state.common.abParticipations, state.page.csrf, (token: string) => dispatch(setGuestAccountCreationToken(token)), (thankYouPageStage: ThankYouPageStage) => dispatch(setThankYouPageStage(thankYouPageStage))), paymentAuthorisation));
}
  ...
  ONE_OFF: {
      ...
      if (paymentAuthorisation.paymentMethod === Stripe) {
        if (paymentAuthorisation.paymentMethodId) {
          ...
          if (handle3DS) {
            ...
            return dispatch(makeCreateStripePaymentIntentRequest(stripeData, (token: string) => dispatch(setGuestAccountCreationToken(token)), (thankYouPageStage: ThankYouPageStage) => dispatch(setThankYouPageStage(thankYouPageStage)), handle3DS, paymentAuthorisation));
          }
    ...
    AmazonPay: (dispatch: Dispatch<Action>, state: State, paymentAuthorisation: PaymentAuthorisation): Promise<PaymentResult> => {
      if (paymentAuthorisation.paymentMethod === AmazonPay && paymentAuthorisation.orderReferenceId !== undefined) {
        return dispatch(executeAmazonPayOneOffPayment(amazonPayDataFromAuthorisation(paymentAuthorisation, state), (token: string) => dispatch(setGuestAccountCreationToken(token)), (thankYouPageStage: ThankYouPageStage) => dispatch(setThankYouPageStage(thankYouPageStage)), paymentAuthorisation));
      }
...
export { updateContributionTypeAndPaymentMethod, updatePaymentMethod, updateSelectedExistingPaymentMethod, updateFirstName, updateLastName, updateEmail, updateBillingState, updateBillingCountry, updateUserFormData, setThirdPartyPaymentLibrary, setAmazonPayHasBegunLoading, setAmazonPayLoginObject, setAmazonPayPaymentsObject, setAmazonPayWalletIsStale, setAmazonPayHasAccessToken, setAmazonPayFatalError, setAmazonPayOrderReferenceId, setAmazonPayBillingAgreementId, setAmazonPayBillingAgreementConsentStatus, setAmazonPayPaymentSelected, selectAmount, updateOtherAmount, paymentFailure, paymentWaiting, paymentSuccess, onThirdPartyPaymentAuthorised, setCheckoutFormHasBeenSubmitted, setGuestAccountCreationToken, setThankYouPageStage, setPasswordHasBeenSubmitted, setPasswordError, updatePassword, createOneOffPayPalPayment, setHasSeenDirectDebitThankYouCopy, checkIfEmailHasPassword, setFormIsValid, sendFormSubmitEventForPayPalRecurring, setPaymentRequestButtonPaymentMethod, setStripePaymentRequestObject, setStripePaymentRequestButtonClicked, setStripePaymentRequestButtonError, setTickerGoalReached, setCreateStripePaymentMethod, setHandleStripe3DS, setStripeCardFormComplete, setStripeSetupIntentClientSecret, setStripeRecurringRecaptchaVerified, setPayPalHasBegunLoading, updatePayPalButtonReady, updateRecaptchaToken, loadPayPalExpressSdk, loadAmazonPaySdk };