...
const useTestimonials = () => {
  ...
  React.useEffect(() => {
    fetch(testimonialsEndpoint).then(response => response.json()).then(data => data.sheets).then(testimonialsData => setTestimonials(testimonialsData));
  }, []);
  return testimonials;
};
...
const AusMomentMap = () => {
  ...
  const animationVariant = () => windowWidthIsGreaterThan('desktop') && selectedTerritory ? 'active' : 'initial';
  ...
  const createTestimonialsContainer = () => <TestimonialsContainer testimonialsCollection={testimonials} selectedTerritory={selectedTerritory} shouldScrollIntoView={shouldScrollIntoView} setSelectedTerritory={territory => {
  ...
  return <div className="map-page">
      <Header />
      <div className="main">
        <motion.div className="left" variants={mapVariants} animate={animationVariant()} transition={animationTransition} positionTransition>
          {windowWidthIsLessThan('desktop') && <Blurb />}
          <Map selectedTerritory={selectedTerritory} setSelectedTerritory={territory => {
          setSelectedTerritory(territory);
          setShouldScrollIntoView(true);
        }} ref={mapRef} />
          <p className="map-caption">Tap the map to read messages from supporters</p>
          <motion.div className="left-padded-inner" transition={animationTransition} animate={animationVariant()} variants={blurbVariants}>
            <Blurb slim />
          </motion.div>
        </motion.div>
        <div className="right">
          {windowWidthIsGreaterThan('desktop') && <Blurb />}
          {windowWidthIsGreaterThan('desktop') ? <motion.div className="testimonials-overlay" {...testimonialsProps()}>
