...
import { TestimonialsCollection, Testimonial } from "pages/aus-moment-map/types/testimonials";
import { Button } from "@guardian/src-button";
import { useWindowWidth } from "../hooks/useWindowWidth";
import { contributeUrl } from "../utils";

const TestimonialCtaPrimary = () => <div className="testimonial-cta testimonial-cta-primary">
    <h3>Are you a supporter?</h3>
    <p>
    If you’re a contributor or subscriber, we would love to hear from you
    </p>

    <LinkButton className="testimonial-cta-primary-link-button" priority="primary" size="small" href="https://www.surveymonkey.co.uk/r/C93WWTC" target="_blank" rel="noopener noreferrer">
      Add your message
    </LinkButton>
  </div>;

const TestimonialCtaSecondary = () => <div className="testimonial-cta testimonial-cta-secondary">
    <h3>Do something powerful today</h3>

    <LinkButton className="testimonial-cta-secondary-link-button" priority="primary" size="small" href={contributeUrl('Aus_Moment_2020_map_inline')} target="_blank" rel="noopener noreferrer">Support the Guardian
    </LinkButton>
  </div>;
...
const TestimonialComponent = (props: TestimonialComponentProps) => {
  ...
  return <div className="testimonial-component">
      <div>
...
const LocationMarker = () => <svg width="12" height="22" viewBox="0 0 12 22" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const TestimonialsForTerritory = (props: TestimonialsForTerritoryProps) => {
  ...
  const secondColumn = props.testimonials.slice(midPointIndex + 1).map(testimonial => <TestimonialComponent testimonial={testimonial} />);
  ...
  React.useEffect(() => {
    if (ref.current) {
      ...
      if (testimonialsContainer) {
        const onScroll = () => {
          if (testimonialsContainer.scrollTop >= offsetTop && testimonialsContainer.scrollTop < offsetTop + offsetHeight) {
  ...
  const displayName = props.territory === 'ACT' && windowWidthIsLessThan('mobileMedium') ? props.territory : TERRITORY_CODE_TO_FULL_NAME[props.territory];
  return <div className="testimonials-for-territory" ref={ref} id={props.territory}>
      ...
      {windowWidthIsGreaterThan('tablet') ? <TestimonialsTwoColumns firstColumn={firstColumn} secondColumn={secondColumn} /> : <TestimonialsExpandableSingleColumn testimonials={[...firstColumn, ...secondColumn]} />}
    </div>;
};
...
const TestimonialsExpandableSingleColumn = (props: TestimonialsExpandableSingleColumnProps) => {
  ...
  return <>
      ...
      <div className="testimonials-read-more-button-container">
        {!isExpanded && <Button priority="tertiary" size="small" onClick={() => {
        setIsExpanded(true);
      }}>
          Read more
        </Button>}
...
const TestimonialsTwoColumns = (props: TestimonialTwoColumnsProps) => <div className="testimonials-columns-container">
    <div className="testimonials-first-column">
      {props.firstColumn}
    </div>
...
const TestimonialsContainerHeader = () => {
  ...
  return <div className="testimonials-container-header" ref={self}>
...
export const TestimonialsContainer = React.forwardRef((props: Props, ref: React.Ref<typeof TestimonialsContainer>) => {
  ...
  if (props.testimonialsCollection) {
    if (props.selectedTerritory || windowWidthIsLessThan('desktop')) {
      const testimonialsForTerritories = Object.keys(props.testimonialsCollection).map(territory => <TestimonialsForTerritory testimonials={props.testimonialsCollection[territory]} territory={territory} shouldScrollIntoView={props.shouldScrollIntoView} selectedTerritory={props.selectedTerritory} setSelectedTerritory={props.setSelectedTerritory} />);
      const midPointCtas = <div className="midpoint-ctas">
