...
const TestimonialCtaPrimary = () => (
	<div className="testimonial-cta testimonial-cta-primary">
		...
		<p>If you’re a contributor or subscriber, we would love to hear from you</p>
...
const TestimonialsForTerritory = (props: TestimonialsForTerritoryProps) => {
	...
	React.useEffect(() => {
		if (ref.current) {
			const onScroll = () => {
				const testimonialsHeader = ref.current.querySelector(
					'.testimonials-for-territory-header',
				);
				const testimonialsContainer =
					testimonialsHeader.parentNode.parentNode.parentNode;
				const testimonialsContainerHeader = testimonialsContainer.querySelector(
...
const TestimonialsExpandableSingleColumn = (
...
export const TestimonialsContainer = React.forwardRef(
	(props: Props, ref: React.Ref<typeof TestimonialsContainer>) => {
		...
		if (props.testimonialsCollection) {
			if (props.selectedTerritory || windowWidthIsLessThan('desktop')) {
				const testimonialsForTerritories = Object.keys(
