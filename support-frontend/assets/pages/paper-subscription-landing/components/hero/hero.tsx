...
function PaperHero({
  ...
  const {
    roundel
  } = getDiscountCopy(maxSavingVsRetail);
  ...
  const copy = promotionHTML(promotionCopy.description, {
    tag: 'p'
  }) || defaultCopy;
  const roundelText = promotionHTML(promotionCopy.roundel) || defaultRoundelText;
  return <PageTitle title="Newspaper subscription" theme="paper" cssOverrides={fitHeadline}>
      <CentredContainer>
        <Hero image={<GridImage gridId="printCampaignHeroHD" srcSizes={[1000, 500, 140]} sizes="(max-width: 740px) 100%,
            (max-width: 1067px) 150%,
            500px" imgType="png" altText="Newspapers" />} hideRoundelBelow="mobileMedium" roundelElement={<HeroRoundel cssOverrides={roundelOffset}>
              {roundelText}
            </HeroRoundel>}>
