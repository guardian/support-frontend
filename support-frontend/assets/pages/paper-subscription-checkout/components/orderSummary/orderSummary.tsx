...
function PaperOrderSummary(props: PropTypes) {
  ...
  const basePaperPrice = props.includesDigiSub ? getPriceWithDiscount(props.productPrices, props.fulfilmentOption, paperProductsWithoutDigital[props.productOption]) : props.total;
  // This allows us to get the price without promotion, so we can say what the price will revert to
  const paperWithoutPromo = getProductPrice(props.productPrices, props.fulfilmentOption, props.productOption);
  ...
  const promotionPriceString = activePromo && monthsDiscounted ? ` for ${monthsDiscounted} months, then ${showPrice(paperWithoutPromo, false)} per month` : '';
