...
const AppStoreImage = (props: {
  store: string;
    ...
    <GridImage classModifiers={['']} gridId={props.store} srcSizes={[140, 500]} sizes="(max-width: 480px) 100px,
                (max-width: 740px) 100%,
...
const AppStoreLink = ({
  ...
  children,
  onClick
}: AppStoreLinkPropTypes) => <a href={storeLink} onClick={onClick} aria-label={ariaLabel}>
    {children}
  </a>;
...
const AppsSection = ({
  countryGroupId
    ...
    <div css={maxWidth}>
      ...
      <AppStoreLink storeLink={getDailyEditionUrl(countryGroupId)} onClick={sendTrackingEventsOnClick({
      ...
      <AppStoreLink storeLink={androidDailyUrl} onClick={sendTrackingEventsOnClick({
      ...
      <AppStoreLink storeLink={getIosAppUrl(countryGroupId)} onClick={sendTrackingEventsOnClick({
      ...
      <AppStoreLink storeLink={androidAppUrl} onClick={sendTrackingEventsOnClick({
      ...
      <ThemeProvider theme={buttonReaderRevenue}>
        <LinkButton css={marginForButton} priority="tertiary" size="default" icon={<SvgArrowRightStraight />} iconSide="right" nudgeIcon aria-label="Click to sign in to the website" href="https://www.theguardian.com/" onClick={sendTrackingEventsOnClick({
