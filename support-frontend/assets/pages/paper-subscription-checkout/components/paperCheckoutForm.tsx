...
function PaperCheckoutForm(props: PropTypes) {
  ...
  useEffect(() => {
    ...
    const plusPrice = includesDigiSub ? props.total : getProductPrice(props.productPrices, props.fulfilmentOption, paperProductsWithDigital[props.productOption]);
    // Price of the standard paper-only product that corresponds to the selected product option
    const paperPrice = includesDigiSub ? getProductPrice(props.productPrices, props.fulfilmentOption, paperProductsWithoutDigital[props.productOption]) : props.total;
      ...
      <Layout aside={isHomeDelivery ? homeDeliveryOrderSummary : subsCardOrderSummary}>
        ...
        props.submitForm();
          ...
          {paymentMethods.length > 1 ? <FormSection cssOverrides={removeTopBorder} title="How would you like to pay?">
              <PaymentMethodSelector currencyId="GBP" paymentMethod={props.paymentMethod} setPaymentMethod={props.setPaymentMethod} validationError={firstError('paymentMethod', props.formErrors)} />
            </FormSection> : null}
