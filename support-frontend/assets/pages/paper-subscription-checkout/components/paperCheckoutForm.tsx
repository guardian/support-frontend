...
import { ProductPrices, ProductPrice } from "helpers/productPrice/productPrices";
...
function mapStateToProps(state: WithDeliveryCheckoutState) {
  return { ...getFormFields(state),
    ...
    total: getPriceWithDiscount(state.page.checkout.productPrices, state.page.checkout.fulfilmentOption, state.page.checkout.productOption),
    amount: getProductPrice(state.page.checkout.productPrices, state.page.checkout.fulfilmentOption, state.page.checkout.productOption)
  };
}
...
function PaperCheckoutForm(props: PropTypes) {
  ...
  function addDigitalSubscription(event: React.SyntheticEvent<HTMLInputElement>) {
  ...
  useEffect(() => {
    ...
    const plusPrice = includesDigiSub ? props.total : getPriceWithDiscount(props.productPrices, props.fulfilmentOption, paperProductsWithDigital[props.productOption]);
    // Price of the standard paper-only product that corresponds to the selected product option
    const paperPrice = includesDigiSub ? getPriceWithDiscount(props.productPrices, props.fulfilmentOption, paperProductsWithoutDigital[props.productOption]) : props.total;
  ...
  const subsCardOrderSummary = <PaperOrderSummary image={<GridImage gridId="printCampaignDigitalVoucher" srcSizes={[500]} sizes="(max-width: 740px) 50vw, 696" imgType="png" altText="" />} total={props.total} digiSubPrice={expandedPricingText} startDate={formattedStartDate} includesDigiSub={includesDigiSub} changeSubscription={`${paperSubsUrl(false, getQueryParameter('promoCode'))}#subscribe`} />;
  const homeDeliveryOrderSummary = <PaperOrderSummary image={<GridImage gridId="printCampaignHDdigitalVoucher" srcSizes={[500]} sizes="(max-width: 740px) 50vw, 696" imgType="png" altText="" />} total={props.total} digiSubPrice={expandedPricingText} includesDigiSub={includesDigiSub} changeSubscription={`${paperSubsUrl(true, getQueryParameter('promoCode'))}#subscribe`} productType={Paper} paymentStartDate={formattedStartDate} />;
      ...
      <Layout aside={isHomeDelivery ? homeDeliveryOrderSummary : subsCardOrderSummary}>
        ...
        props.submitForm();
          ...
          <FormSection title="Your details">
            ...
            <PersonalDetails firstName={props.firstName} setFirstName={props.setFirstName} lastName={props.lastName} setLastName={props.setLastName} email={props.email} telephone={props.telephone} setTelephone={props.setTelephone} formErrors={props.formErrors} signOut={props.signOut} />
          </FormSection>

          <FormSection title={deliveryTitle}>
            <DeliveryAddress />
            {isHomeDelivery ? <TextArea css={controlTextAreaResizing} id="delivery-instructions" label="Delivery instructions" autocomplete="new-password" // Using "new-password" here because "off" isn't working in chrome
          supporting="Please let us know any details to help us find your property (door colour, any access issues) and the best place to leave your newspaper. For example, 'Front door - red - on Crinan Street, put through letterbox'" maxlength={250} value={props.deliveryInstructions} onChange={e => props.setDeliveryInstructions(e.target.value)} optional /> : null}
          </FormSection>
          ...
          <FormSection title="Is the billing address the same as the delivery address?">
            <Rows>
              <RadioGroup label="Is the billing address the same as the delivery address?" hideLabel id="billingAddressIsSame" name="billingAddressIsSame" orienntation="vertical" error={firstError('billingAddressIsSame', props.formErrors)}>
                <Radio inputId="qa-billing-address-same" value="yes" label="Yes" name="billingAddressIsSame" checked={props.billingAddressIsSame === true} onChange={() => props.setBillingAddressIsSame(true)} />

                <Radio id="qa-billing-address-different" label="No" value="no" name="billingAddressIsSame" checked={props.billingAddressIsSame === false} onChange={() => props.setBillingAddressIsSame(false)} />
              </RadioGroup>
            </Rows>
          </FormSection>
          ...
          {isHomeDelivery ? <FormSection title="When would you like your subscription to start?">
              <Rows>
                <RadioGroup label="When would you like your subscription to start?" hideLabel id="startDate" error={firstError('startDate', props.formErrors)} legend="When would you like your subscription to start?">
                  {days.map(day => {
          ...
          {paymentMethods.length > 1 ? <FormSection cssOverrides={removeTopBorder} title="How would you like to pay?">
              <PaymentMethodSelector country={props.country} paymentMethod={props.paymentMethod} setPaymentMethod={props.setPaymentMethod} validationError={firstError('paymentMethod', props.formErrors)} />
            </FormSection> : null}
          <FormSectionHiddenUntilSelected id="stripeForm" show={props.paymentMethod === Stripe} title="Your card details">
            <StripeProviderForCountry country={props.country} isTestUser={props.isTestUser} submitForm={props.submitForm} allErrors={[...props.billingAddressErrors, ...props.deliveryAddressErrors, ...props.formErrors]} setStripePaymentMethod={props.setStripePaymentMethod} name={`${props.firstName} ${props.lastName}`} validateForm={props.validateForm} buttonText="Pay now" csrf={props.csrf} />
