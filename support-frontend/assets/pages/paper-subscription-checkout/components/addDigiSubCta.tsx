...
type PropTypes = {
  digiSubPrice: string;
  addDigitalSubscription: (event: React.SyntheticEvent<HTMLInputElement>) => void;
};
...
function AddDigiSubCta({
  ...
  return <Accordion cssOverrides={[ctaContainer, expanded ? ctaContainerOpen : '']} hideToggleLabel>
      <AccordionRow cssOverrides={rowOverrides} label={`Would you like to add a digital subscription for ${digiSubPrice}?`} onClick={() => {
      setExpanded(!expanded);
    }}>
        <div css={[imageContainer, lightBorder]}>
          <GridImage gridId="editionsShortPackshot" srcSizes={[500, 140]} sizes="(max-width: 480px) 200px,
            (max-width: 740px) 100%,
            500px" altText="Digital subscriptions" imgType="png" />
        </div>
        ...
        <div css={content}>
          <Checkbox value="add-digital" label={`Add the digital subscription for ${digiSubPrice}`} onChange={addDigitalSubscription} />
        </div>
      </AccordionRow>
    </Accordion>;
}
