...
import { FormFields } from "helpers/subscriptionsForms/formFields";
...
const whatNextText: Record<FulfilmentOptions, Array<string>> = {
  [HomeDelivery]: ['Look out for an email from us confirming your subscription. It has everything you need to know about how to manage it in the future. As well as future communications on how  to make the most of your subscription and weekly newsletters written by the editors. You can opt out at any time via your account.', 'Your newspaper will be delivered to your door.'],
  [Collection]: [`Keep an eye on your inbox. You should receive an email confirming the details of your subscription,
      and another email shortly afterwards that contains details of how you can pick up your newspapers from tomorrow.`, `You will receive your Subscription Card in your subscriber pack in the post, along with your home
      delivery letter.`, `Visit your chosen participating newsagent to pick up your newspaper using your Subscription Card, or
...
const MyAccountLink = () => <a href={myAccountUrl} onClick={sendTrackingEventsOnClick({
...
function ThankYouContent({
  ...
  return <div className="thank-you-stage">
      <HeroWrapper appearance="custom" className={styles.hero}>
        <HeroPicture />
        <HeadingBlock overheading="Thank you for supporting our journalism!" overheadingClass="--thankyou">
          {isPending ? `Your subscription to the ${packageName}is being processed` : `You have now subscribed to the ${packageName}`}
        </HeadingBlock>
      </HeroWrapper>
      {showTopContentBlock && <Content divider>
          {isPending && <p css={subHeading}>
                  Your subscription is being processed and you will
                  receive an email when it goes live.
              </p>}
          {startDate && !hideStartDate && <p css={subHeading}>
              <span>
      ...
      {hasAddedDigitalSubscription && <Content>
