// ----- Imports ----- //
import { getPriceDescription, getAppliedPromoDescription, getSimplifiedPriceDescription, getAdverbialSubscriptionDescription } from "helpers/productPrice/priceDescriptions";
...
describe('getPriceDescription', () => {
  it('should return a price based on inputs', () => {
    ...
    expect(getPriceDescription(gwAnnual, Annual)).toEqual('You\'ll pay £135 for 1 year, then £150 per year');
    const gwQuarterly = {
      price: 37.5,
      currency: 'GBP',
      promotions: []
    };
    expect(getPriceDescription(gwQuarterly, Quarterly)).toEqual('£37.50 per quarter');
    const gwMonthly = {
      price: 12.5,
      currency: 'GBP',
      promotions: []
    ...
    expect(getPriceDescription(gwQuarterlyWithPromo, Quarterly)).toEqual('You\'ll pay £31.87 for 1 quarter, then £37.50 per quarter');
    const gwSixWeekly = {
      price: 27.50,
      currency: 'USD',
      promotions: [{
    ...
    expect(getPriceDescription(gwSixWeekly, SixWeekly)).toEqual('US$6 for the first 6 issues (then US$27.50 per month)');
  });
});
describe('getAppliedPromoDescription', () => {
  ...
  it('should return a landing page promotion roundel description', () => {
    expect(getAppliedPromoDescription(monthlyBillingPeriod, productPriceWithLandingPageDiscount)).toBe('Save 50% for 3 months!');
  });
});
