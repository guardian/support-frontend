...
const fetchFonts = (window: Record<string, any>, document: Document): void => {
  ...
  const saveFontToLocalStorage = (font: {
    fontName: string;
    css: string;
  }) => {
    const {
    ...
    if (fontName) {
      ...
      storage.setLocal('guFonts', JSON.stringify({ ...currentFonts,
        [fontName]: font
      }));
    }
  };
  ...
  const loadFonts = (): void => {
    ...
    window.addEventListener('message', (e: MessageEvent) => {
      if (iframe instanceof HTMLIFrameElement && e && e.data && e.data.name && e.data.name === 'guardianFonts' && e.data.fonts && e.source === iframe.contentWindow) {
