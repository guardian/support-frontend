...
const setupSubscriptionPayPalPayment = (resolve: (arg0: string) => void, reject: (arg0: Error) => void, currency: IsoCurrency, csrf: CsrfState, amount: number, billingPeriod: BillingPeriod) => (): void => {
  const csrfToken = csrf.token;
  storage.setSession('selectedPaymentMethod', PayPal);
  const requestBody = {
    amount,
...
function createAgreement(payPalData: Record<string, any>, csrf: CsrfState) {
  ...
  const csrfToken = csrf.token;
  return fetch(routes.payPalCreateAgreement, payPalRequestData(body, csrfToken || '')).then(response => response.json());
}

function getPayPalOptions(currencyId: IsoCurrency, csrf: CsrfState, onPaymentAuthorisation: (arg0: string) => void, canOpen: () => boolean, onClick: () => void, formClassName: string, isTestUser: boolean, amount: number, billingPeriod: BillingPeriod, setupPayPalPayment: SetupPayPalRequestType, updatePayPalButtonReady: (arg0: boolean) => Action): Record<string, any> {
  ...
  return {
    ...
    onAuthorize: data => {
      createAgreement(data, csrf).then((baid: Record<string, any>) => {
...
// ----- Exports ----- //
export { getPayPalOptions, showPayPal, loadPayPalRecurring, payPalRequestData, setupSubscriptionPayPalPayment, setupRecurringPayPalPayment };