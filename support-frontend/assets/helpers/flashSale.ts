...
type SaleDetails = Record<CountryGroupId, {
  promoCode: string;
...
const Sales: Sale[] = [{
  ...
  startTime: new Date(2019, 8, 23).getTime(),
  // 23 Sept 2019
  endTime: new Date(2020, 8, 23).getTime(),
  // 23 Sept 2020 - we don't have an end date yet
  saleDetails: {
    GBPCountries: dpSale,
    UnitedStates: { ...dpSale,
      price: 9.99
...
function getActiveFlashSales(product: SubscriptionProduct, countryGroupId: CountryGroupId = detect()): Sale[] {
  ...
  const sales = Sales.filter(sale => sale.subscriptionProduct === product && sale.activeRegions.includes(countryGroupId)).sort(sortSalesByStartTimesDescending);
  return sales.filter(sale => now > sale.startTime && now < sale.endTime || getFlashSaleActiveOverride() === true);
}

function getDiscount(product: SubscriptionProduct, countryGroupId: CountryGroupId): number | null | undefined {
...
function getDuration(product: SubscriptionProduct, countryGroupId: CountryGroupId): string | null | undefined {
  const sale = getActiveFlashSales(product, countryGroupId)[0];
...
function getPromoCode(product: SubscriptionProduct, countryGroupId: CountryGroupId = detect(), defaultCode: string): string {
...
function getIntcmp(product: SubscriptionProduct, countryGroupId: CountryGroupId = detect(), intcmp: string | null | undefined, defaultIntcmp: string): string {
...
function getDisplayFlashSalePrice(product: SubscriptionProduct, countryGroupId: CountryGroupId, period: BillingPeriod): string {
...
export { flashSaleIsActive, getDisplayFlashSalePrice, getPromoCode, getIntcmp, getEndTime, getSaleCopy, getFormattedFlashSalePrice, getDiscount, getDuration, getTimeTravelDaysOverride, getFlashSaleActiveOverride, countdownTimerIsActive, showCountdownTimer, dpSale };