...
const fetchFonts = (window: Record<string, any>, document: Document): void => {
	...
	const useFont = (font: { fontName: string; css: string }): void => {
		...
		if (
			document.getElementsByClassName(fontClassName).length === 0 &&
			head &&
			font.fontName &&
			font.css
		) {
	...
	const saveFontToLocalStorage = (font: { fontName: string; css: string }) => {
		...
		if (fontName) {
			...
			const currentFonts = currentFontsString
				? JSON.parse(currentFontsString)
				: {};
			storage.setLocal(
				'guFonts',
				JSON.stringify({ ...currentFonts, [fontName]: font }),
			);
		}
	};
