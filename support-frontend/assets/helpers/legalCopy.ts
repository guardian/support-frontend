...
export const productLegal = (
	...
	const amountPerPeriod = `${amountFormatted}${periodDivider}${periodNoun}`;
	...
	if (promotion) {
		...
		return `${promoPriceFormatted}${periodDivider}${periodNoun} for the first ${
			discountDuration > 1 ? discountDuration : ''
		} ${periodNoun}${discountDuration > 1 ? 's' : ''}${
			amountDivider ?? ','
