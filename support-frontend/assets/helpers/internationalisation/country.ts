...
import { AUDCountries, Canada, countryGroups, EURCountries, GBPCountries, International, NZDCountries, UnitedStates } from "./countryGroup";
...
const stripePaymentRequestAllowedCountries = ['AT', 'AU', 'BE', 'BR', 'CA', 'CH', 'DE', 'DK', 'EE', 'ES', 'FI', 'FR', 'GB', 'HK', 'IE', 'IN', 'IT', 'JP', 'LT', 'LU', 'LV', 'MX', 'NL', 'NZ', 'NO', 'PH', 'PL', 'PT', 'RO', 'SE', 'SG', 'SK', 'US'];
...
// ----- Functions ----- /
function stateProvinceFromMap(l: string, states: Record<string, string>): StateProvince | null | undefined {
  const s = l.toUpperCase();
  return states[s] ? s : Object.keys(states).find(key => states[key].toUpperCase() === s || s.length === 3 && s.startsWith(key));
}
...
function stateProvinceFieldFromString(countryGroupId: CountryGroupId | null | undefined, s?: string): Option<StateProvince> {
  if (!s) {
    return null;
  }
...
function fromCountryGroup(countryGroupId: CountryGroupId | null | undefined = null): IsoCountry | null | undefined {
  switch (countryGroupId) {
    case UnitedStates:
      return 'US';
...
function fromPath(path: string = window.location.pathname): IsoCountry | null | undefined {
...
type TargetCountryGroups = typeof International | typeof EURCountries | typeof NZDCountries | typeof GBPCountries | typeof AUDCountries;

function handleCountryForCountryGroup(targetCountryGroup: TargetCountryGroups, countryGroupId: CountryGroupId | null | undefined = null): IsoCountry | null | undefined {
  ...
  if (path !== paths[targetCountryGroup][0] && !path.startsWith(paths[targetCountryGroup][1]) && countryGroupId !== targetCountryGroup) {
    return null;
  }

  const candidateCountry: IsoCountry | null | undefined = fromQueryParameter() || fromCookie() || fromGeolocation();
...
function detect(countryGroupId: CountryGroupId | null | undefined = null): IsoCountry {
  ...
  if (country === null) {
    country = fromCountryGroup(countryGroupId) || fromPath() || fromQueryParameter() || fromCookie() || fromGeolocation() || 'GB';
  }
...
// ----- Exports ----- //
export { detect, setCountry, usStates, caStates, auStates, countries, newspaperCountries, findIsoCountry, fromString, fromGeolocation, stateProvinceFieldFromString, stateProvinceFromString, fromCountryGroup };