...
export type CountryGroupId = typeof GBPCountries | typeof UnitedStates | typeof AUDCountries | typeof EURCountries | typeof International | typeof NZDCountries | typeof Canada;
...
// ----- Functions ----- //
function fromPath(path: string = window.location.pathname): CountryGroupId | null | undefined {
...
function fromCountry(isoCountry: string): CountryGroupId | null | undefined {
  ...
  const countryGroup = Object.keys(countryGroups).find(countryGroupId => countryGroups[countryGroupId].countries.includes(isoCountry));
  return countryGroup || null;
}

function fromQueryParameter(): CountryGroupId | null | undefined {
  const countryGroup: string | null | undefined = getQueryParameter('countryGroup');
...
function fromCountryGroupName(name: CountryGroupName): CountryGroup {
  const groupId: CountryGroupId | null | undefined = Object.keys(countryGroups).find(key => countryGroups[key].name === name);
  return groupId ? countryGroups[groupId] : countryGroups.GBPCountries;
}

// ----- Exports ----- //
export { countryGroups, detect, stringToCountryGroupId, fromCountry, fromCountryGroupName, GBPCountries, UnitedStates, AUDCountries, EURCountries, NZDCountries, Canada, International };