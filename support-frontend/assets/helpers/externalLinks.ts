...
type PromoCodes = Record<SubscriptionProduct, string>;
...
const customPromos: Record<Campaign, PromoCodes> = {
  seven_fifty_middle: {
...
function getMemLink(product: MemProduct, intCmp: string | null | undefined): string {
...
function buildParamString(product: SubscriptionProduct, countryGroupId: CountryGroupId, intCmp: string | null | undefined, referrerAcquisitionData: ReferrerAcquisitionData | null): string {
...
function buildSubsUrls(countryGroupId: CountryGroupId, promoCodes: PromoCodes, intCmp: string | null | undefined, referrerAcquisitionData: ReferrerAcquisitionData): SubsUrls {
...
function getSubsLinks(countryGroupId: CountryGroupId, intCmp: string | null | undefined, campaign: Campaign | null | undefined, referrerAcquisitionData: ReferrerAcquisitionData, nativeAbParticipations: Participations): SubsUrls {
  const acquisitionData = deriveSubsAcquisitionData(referrerAcquisitionData, nativeAbParticipations);

  if (campaign && customPromos[campaign]) {
    return buildSubsUrls(countryGroupId, customPromos[campaign], intCmp, acquisitionData);
  }
...
function getDigitalCheckout(countryGroupId: CountryGroupId, billingPeriod: DigitalBillingPeriod | DigitalGiftBillingPeriod, promoCode: Option<string>, orderIsAGift: boolean): string {
  const params = new URLSearchParams(window.location.search);
...
const getProfileUrl = (path: string) => (returnUrl: string | null | undefined) => {
...
// ----- Exports ----- //
export { getSubsLinks, getMemLink, getPatronsLink, getDigitalCheckout, getIosAppUrl, androidAppUrl, androidDailyUrl, getDailyEditionUrl, getSignoutUrl, getReauthenticateUrl, myAccountUrl, manageSubsUrl, homeDeliveryUrl, helpCentreUrl };