...
import { PaymentMethod, PaymentMethodMap } from "helpers/forms/paymentMethods";
...
export const contributionTypeIsRecurring = (contributionType: ContributionType) => contributionType === 'MONTHLY' || contributionType === 'ANNUAL';
...
export type ContributionTypes = Record<CountryGroupId, ContributionTypeSetting[]>;
...
export type Config = Record<ContributionType, {
  min: number;
  minInWords: string;
  max: number;
  maxInWords: string;
...
export type OtherAmounts = Record<ContributionType, {
  amount: string | null;
}>;
export type SelectedAmounts = Record<ContributionType, number | "other">;

const getAmount = (selectedAmounts: SelectedAmounts, otherAmounts: OtherAmounts, contributionType: ContributionType) => parseFloat(selectedAmounts[contributionType] === 'other' ? otherAmounts[contributionType].amount : selectedAmounts[contributionType]);
...
// ----- Functions ----- //
function validateContribution(input: number, contributionType: ContributionType, countryGroupId: CountryGroupId): ValidationError | null | undefined {
...
function toContributionType(s: string | null | undefined): ContributionType | null | undefined {
  if (s) {
...
function errorMessage(error: ContributionError, contributionType: ContributionType, countryGroupId: CountryGroupId): string | null | undefined {
...
function getCustomAmountA11yHint(contributionType: ContributionType, countryGroupId: CountryGroupId): string {
...
function getAmountA11yHint(contributionType: ContributionType, currencyId: IsoCurrency, spokenAmount: string): string {
...
function getContributionAmountRadios(amounts: number[], contributionType: ContributionType, currencyId: IsoCurrency): Radio[] {
...
const contributionTypeAvailable = (contributionType: ContributionType, countryGroupId: CountryGroupId, contributionTypes: ContributionTypes): boolean => contributionTypes[countryGroupId].some(settings => settings.contributionType === contributionType);

// ----- Exports ----- //
export { config, toContributionType, generateContributionTypes, validateContribution, parseContribution, getMinContribution, billingPeriodFromContrib, errorMessage, getContributionTypeClassName, getSpokenType, getFrequency, getCustomAmountA11yHint, contributionTypeRadios, getContributionAmountRadios, parseRegularContributionType, getAmount, contributionTypeAvailable };