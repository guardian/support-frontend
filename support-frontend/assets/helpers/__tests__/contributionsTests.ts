// ----- Imports ----- //
import { validateContribution, parseContribution, getMinContribution } from "../contributions";
...
describe('contributions', () => {
  describe('validateContribution', () => {
    it('should return "TooLittle" for contributions below the minimum', () => {
      [[1, 'MONTHLY', GBPCountries], [5, 'ANNUAL', AUDCountries], [0.5, 'ONE_OFF', EURCountries]].forEach(a => expect(validateContribution(...a)).toBe('TooLittle'));
    });
    it('should return "TooMuch" for contributions above the maximum', () => {
      [[2000, 'MONTHLY', UnitedStates], [2030, 'ANNUAL', International], [2030, 'ONE_OFF', Canada]].forEach(a => expect(validateContribution(...a)).toBe('TooMuch'));
    });
    it('should return null for contributions that are OK', () => {
      [[100, 'MONTHLY', NZDCountries], [100, 'ANNUAL', UnitedStates], [100, 'ONE_OFF', GBPCountries]].forEach(a => expect(validateContribution(...a)).toBeNull());
    });
  });
