...
function userInBreakpoint(audience: Audience): boolean {
  ...
  const mediaQuery = minWidthMediaQuery && maxWidthMediaQuery ? `${minWidthMediaQuery} and ${maxWidthMediaQuery}` : minWidthMediaQuery || maxWidthMediaQuery;
  return window.matchMedia(mediaQuery).matches;
}

function userInTest(test: Test, testId: string, mvtId: number, country: IsoCountry, countryGroupId: CountryGroupId, acquisitionDataTest: AcquisitionABTest | null | undefined) {
  const {
    audiences,
    referrerControlled
  } = test;
...
function assignUserToVariant(mvtId: number, test: Test, acquisitionDataTest: AcquisitionABTest | null | undefined): number {
  const {
    referrerControlled,
    seed
  } = test;
...
function targetPageMatches(locationPath: string, targetPage: (string | null | undefined) | RegExp) {
  if (!targetPage) {
    return true;
  }
...
function getParticipations(abTests: Tests, mvtId: number, country: IsoCountry, countryGroupId: CountryGroupId): Participations {
  const participations: Participations = {};
  const acquisitionDataTest: AcquisitionABTest | null | undefined = getTestFromAcquisitionData();
...
function getAmountsTestParticipations(countryGroupId: CountryGroupId, settings: Settings): Participations | null | undefined {
  if (!settings.amounts) {
    return null;
  }
...
const init = (country: IsoCountry, countryGroupId: CountryGroupId, settings: Settings, abTests: Tests = tests): Participations => {
  const mvt: number = getMvtId();
  const participations: Participations = getParticipations(abTests, mvt, country, countryGroupId);
  const urlParticipations: Participations | null | undefined = getParticipationsFromUrl();
  const serverSideParticipations: Participations | null | undefined = getServerSideParticipations();
  const amountsTestParticipations: Participations | null | undefined = getAmountsTestParticipations(countryGroupId, settings);
  return { ...participations,
