...
type CampaignCopy = {
  headerCopy?: string | React.ReactElement<React.ComponentProps<string>, string>;
  contributeCopy?: string | React.ReactElement<React.ComponentProps<string>, string>;
};
export type CampaignSettings = {
  campaignCode: string;
  copy?: (goalReached: boolean) => CampaignCopy;
  formMessage?: React.ReactElement<React.ComponentProps<string>, string>;
  termsAndConditions?: (contributionsTermsLink: string, contactEmail: string) => React.ReactElement<React.ComponentProps<string>, string>;
  cssModifiers?: string[];
...
function campaignEnabledForUser(campaignCode: string | null | undefined): boolean {
  if (currentCampaignPath && window.guardian.enableContributionsCampaign) {
    return window.guardian.forceContributionsCampaign || window.location.pathname.endsWith(`/${currentCampaignPath}`) || campaign.campaignCode === campaignCode;
  }

  return false;
}

export function getCampaignSettings(campaignCode: string | null | undefined): CampaignSettings | null {
