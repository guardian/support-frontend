...
function buildReferrerAcquisitionData(acquisitionData: Record<string, any> = {}): ReferrerAcquisitionData {
  ...
  // This was how referrer pageview id used to be passed.
  const campaignCode = getCampaignCode() || acquisitionData.campaignCode || getQueryParameter('INTCMP');
...
function getReferrerAcquisitionData(): ReferrerAcquisitionData {
  const paramData = getQueryParameter(REFERRAL_DATA_PARAM) ? deserialiseReferralData(getQueryParameter(REFERRAL_DATA_PARAM) || '') : deserialiseJsonObject(getQueryParameter(ACQUISITIONS_PARAM) || '');
