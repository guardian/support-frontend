...
const campaigns: Record<string, string[]> = {
  ...
  epic_paradise_paradise_highlight: ['gdnwb_copts_memco_epic_paradise_paradise_highlight'],
  epic_paradise_different_highlight: ['gdnwb_copts_memco_epic_paradise_different_highlight'],
...
function getCampaign(acquisition: ReferrerAcquisitionData): Campaign | null | undefined {
  const {
    campaignCode
  } = acquisition;
  ...
  return Object.keys(campaigns).find(campaign => campaigns[campaign].includes(campaignCode)) || null;
}
...
const toAcquisitionQueryParameters = (parameters: Array<[string, string]>): AcquisitionQueryParameters => parameters.reduce((array, item) => {
  array.push({
    name: item[0],
    value: item[1]
  });
...
function buildReferrerAcquisitionData(acquisitionData: Record<string, any>): ReferrerAcquisitionData {
  // This was how referrer pageview id used to be passed.
  const referrerPageviewId = acquisitionData.referrerPageviewId || getQueryParameter('REFPVID');
  const campaignCode = getCampaignCode() || acquisitionData.campaignCode || getQueryParameter('INTCMP') || getQueryParameter('CMP');
  const parameterExclusions = ['REFPVID', 'INTCMP', 'acquisitionData', 'contributionValue', 'contribType', 'currency'];
  const queryParameters = acquisitionData.queryParameters || toAcquisitionQueryParameters(getAllQueryParamsWithExclusions(parameterExclusions));
...
const getAbTests = (referrerAcquisitionData: ReferrerAcquisitionData, participations: Participations) => {
  const alltests = [...participationsToAcquisitionABTest(participations), ...(referrerAcquisitionData.abTests || [])];
  return alltests.reduce((acc: AcquisitionABTest[], abTest: AcquisitionABTest) => acc.find(test => test.name === abTest.name) ? acc : acc.concat([abTest]), []);
};

function derivePaymentApiAcquisitionData(referrerAcquisitionData: ReferrerAcquisitionData, nativeAbParticipations: Participations): PaymentAPIAcquisitionData {
  const ophanIds: OphanIds = getOphanIds();
  const abTests = getAbTests(referrerAcquisitionData, nativeAbParticipations);
  const campaignCodes = referrerAcquisitionData.campaignCode ? [referrerAcquisitionData.campaignCode] : [];
  return {
    platform: 'SUPPORT',
...
function deriveSubsAcquisitionData(referrerAcquisitionData: ReferrerAcquisitionData, nativeAbParticipations: Participations): ReferrerAcquisitionData {
  const abTests = getAbTests(referrerAcquisitionData, nativeAbParticipations);
...
function getReferrerAcquisitionDataFromSessionStorage(): ReferrerAcquisitionData | null | undefined {
...
function getAcquisitionDataFromAcquisitionDataParam(): Record<string, any> | null | undefined {
...
function getAcquisitionDataFromReferralDataParam(): Record<string, any> | null | undefined {
...
function getAcquisitionDataFromPPCParams(): Record<string, any> | null | undefined {
...
function getReferrerAcquisitionData(): ReferrerAcquisitionData {
  ...
  const candidateAcquisitionData = getAcquisitionDataFromReferralDataParam() || getAcquisitionDataFromAcquisitionDataParam() || getAcquisitionDataFromPPCParams() || getReferrerAcquisitionDataFromSessionStorage();
...
// ----- Exports ----- //
export { getCampaign, getReferrerAcquisitionData, getOphanIds, participationsToAcquisitionABTest, derivePaymentApiAcquisitionData, deriveSubsAcquisitionData, getSupportAbTests };