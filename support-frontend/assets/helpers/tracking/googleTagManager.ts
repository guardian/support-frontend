...
import { PaymentMethod } from "../forms/paymentMethods";
...
type PaymentRequestAPIStatus = "PaymentRequestAPINotAvailable" | "CanMakePaymentNotAvailable" | "AvailableNotInUse" | "AvailableInUse" | "PaymentRequestAPIError" | "PromiseNotSupported" | "PromiseRejected" | "PaymentApiPromiseRejected";
type GaEventData = {
  category: string;
  action: string;
...
function getData(event: EventType, participations: Participations, paymentRequestApiStatus?: PaymentRequestAPIStatus): Record<string, any> {
...
function pushToDataLayer(event: EventType, participations: Participations) {
  try {
    getPaymentAPIStatus().then(paymentRequestApiStatus => {
...
function gaEvent(gaEventData: GaEventData, additionalFields: Record<string, any> | null | undefined) {
  const pushEventToGA = () => {
    push({
      event: 'GAEvent',
...
// ----- Exports ---//
export { init, gaEvent, successfulConversion, appStoreCtaClick, gaPropertyId, mapFields };