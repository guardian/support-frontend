...
describe('acquisitions', () => {
	describe('derivePaymentApiAcquisitionData', () => {
		it('should return a PaymentAPIAcquisitionData correctly built', () => {
			...
			expect(
				paymentApiAcquisitionData.abTests &&
					paymentApiAcquisitionData.abTests.length,
			).toEqual(4);
			expect(
				paymentApiAcquisitionData.campaignCodes &&
					paymentApiAcquisitionData.campaignCodes.length,
			).toEqual(1);
		});
	});
