...
import { NoProductOptions, paperProductsWithDigital, paperProductsWithoutDigital } from "helpers/productPrice/productOptions";
import type { FulfilmentOptions } from "helpers/productPrice/fulfilmentOptions";
import { getWeeklyFulfilmentOption, NoFulfilmentOptions } from "helpers/productPrice/fulfilmentOptions";
import type { FormState } from "helpers/subscriptionsForms/formFields";
import type { Option } from "helpers/types/option";
import { GuardianWeekly } from "helpers/productPrice/subscriptions";

function createFormReducer(initialCountry: IsoCountry, product: SubscriptionProduct, initialBillingPeriod: BillingPeriod, startDate: Option<string>, productOption: Option<ProductOptions>, fulfilmentOption: Option<FulfilmentOptions>) {
  ...
  const getFulfilmentOption = (action, currentOption) => // For GuardianWeekly subs, when the country changes we need to update the fulfilment option
  ...
  return (originalState: FormState = initialState, action: Action): FormState => {
    ...
    switch (action.type) {
      ...
      case 'SET_TITLE_GIFT':
        return { ...state,
          titleGiftRecipient: action.titleGiftRecipient
        };
      ...
      case 'SET_PAYPAL_HAS_LOADED':
        return { ...state,
          payPalHasLoaded: true
        };

      case 'SET_ORDER_IS_GIFT':
        return { ...state,
          orderIsAGift: action.orderIsAGift
        };

      case 'SET_STRIPE_PAYMENT_METHOD':
        return { ...state,
          stripePaymentMethod: action.stripePaymentMethod
        };
      ...
      case 'SET_ADD_DIGITAL_SUBSCRIPTION':
        return { ...state,
          productOption: action.addDigital ? paperProductsWithDigital[state.productOption] : paperProductsWithoutDigital[state.productOption]
        };
