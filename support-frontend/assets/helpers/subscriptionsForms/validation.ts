...
// ----- Rules ----- //
const nonEmptyString: (arg0: string | null | undefined) => boolean = s => (s || '').trim() !== '';
...
function validate<Err>(rules: Rule<Err>[]): Err[] {
  return rules.reduce((errors, {
    rule,
    error
  }) => rule ? errors : [...errors, error], []);
}

// -------- Forms------------ //
// ----- Exports ----- //
export { nonEmptyString, notNull, firstError, formError, removeError, validate, nonSillyCharacters };