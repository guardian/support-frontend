...
import { checkOptionalEmail, checkEmail, checkGiftStartDate, emailAddressesMatch } from "helpers/forms/formValidation";
...
function applyCheckoutRules(fields: FormFields): FormError<FormField>[] {
  ...
  const formFieldsToCheck = orderIsAGift ? [...userFormFields, ...giftFormFields] : userFormFields;
...
function applyDeliveryRules(fields: FormFields): FormError<FormField>[] {
  return validate([{
    ...
    error: formError('billingAddressIsSame', 'Please indicate whether the billing address is the same as the delivery address')
