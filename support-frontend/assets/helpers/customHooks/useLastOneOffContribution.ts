...
export function useLastOneOffContribution(): OneOffContribution | null {
  const [lastOneOffContribution, setlastOneOffContribution] = useState<OneOffContribution | null>(null);
  useEffect(() => {
    fetch(MDAPI_URL, {
      mode: 'cors',
      credentials: 'include'
      ...
      const latest = contributions.filter(c => c.status.toUpperCase() === 'PAID').sort((c1, c2) => c2.created - c1.created)[0];
