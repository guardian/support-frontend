import { PaymentAPIAcquisitionData } from "helpers/tracking/acquisitions";
...
import { ThankYouPageStage, Stripe3DSResult } from "pages/contributions-landing/contributionsLandingReducer";
// ----- Types ----- //
...
function paymentResultFromObject(json: Record<string, any>, setGuestAccountCreationToken: (arg0: string) => void, setThankYouPageStage: (arg0: ThankYouPageStage) => void): Promise<PaymentResult> {
  if (json.error) {
...
const handleOneOffExecution = (result: Promise<Record<string, any>>) => (setGuestAccountCreationToken: (arg0: string) => void, setThankYouPageStage: (arg0: ThankYouPageStage) => void): Promise<PaymentResult> => logPromise(result).then(r => paymentResultFromObject(r, setGuestAccountCreationToken, setThankYouPageStage));
