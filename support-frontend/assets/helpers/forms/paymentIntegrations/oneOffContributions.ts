...
function paymentResultFromObject(
	...
	setGuestAccountCreationToken: (arg0: string) => void,
	setThankYouPageStage: (arg0: ThankYouPageStage) => void,
): Promise<PaymentResult> {
	if (json.error) {
		const failureReason: ErrorReason = json.error.failureReason
			? json.error.failureReason
			: 'unknown';
...
const handleOneOffExecution =
	...
	(
		...
		logPromise(result).then((r) =>
			paymentResultFromObject(
				r,
		...
		postToPaymentApi(data, '/contribute/one-off/stripe/create-payment').then(
			(createIntentResponse) => {
				if (createIntentResponse.type === 'requiresaction') {
					...
					return handleStripe3DS(createIntentResponse.data.clientSecret).then(
