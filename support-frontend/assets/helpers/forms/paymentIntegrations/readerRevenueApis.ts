...
import { AcquisitionABTest, OphanIds, ReferrerAcquisitionData } from "helpers/tracking/acquisitions";
...
import { DigitalPack, GuardianWeekly, Paper } from "helpers/productPrice/subscriptions";
...
import { CaState, IsoCountry, UsState } from "helpers/internationalisation/country";
...
import { ThankYouPageStage } from "pages/contributions-landing/contributionsLandingReducer";
import "pages/contributions-landing/contributionsLandingReducer";
import { DirectDebit, ExistingCard, ExistingDirectDebit, PayPal, Stripe, AmazonPay, Sepa } from "helpers/forms/paymentMethods";
...
export type RegularPaymentFields = RegularPayPalPaymentFields | RegularStripePaymentIntentFields | RegularDirectDebitPaymentFields | RegularSepaPaymentFields | RegularExistingPaymentFields | CorporateRedemption | RegularAmazonPayPaymentFields;
...
export type PaymentAuthorisation = StripePaymentIntentAuthorisation | PayPalAuthorisation | DirectDebitAuthorisation | SepaAuthorisation | ExistingCardAuthorisation | ExistingDirectDebitAuthorisation | AmazonPayAuthorisation;
  ...
  switch (authorisation.paymentMethod) {
    ...
    case AmazonPay:
      if (authorisation.amazonPayBillingAgreementId) {
        return {
          amazonPayBillingAgreementId: authorisation.amazonPayBillingAgreementId
        };
      }
 ...
 */
function checkRegularStatus(participations: Participations, csrf: CsrfState, setGuestAccountCreationToken: (arg0: string) => void, setThankYouPageStage: (arg0: ThankYouPageStage) => void): (arg0: Record<string, any>) => Promise<PaymentResult> {
  ...
  return json => {
    ...
    switch (json.status) {
      case 'pending':
        return logPromise(pollUntilPromise(MAX_POLLS, POLLING_INTERVAL, () => fetchJson(json.trackingUri, getRequestOptions('same-origin', csrf)), json2 => json2.status === 'pending').then(handleCompletion, handleExhaustedPolls));
...
function postRegularPaymentRequest(uri: string, data: RegularPaymentRequest, participations: Participations, csrf: CsrfState, setGuestAccountCreationToken: (arg0: string) => void, setThankYouPageStage: (arg0: ThankYouPageStage) => void): Promise<PaymentResult> {
  return logPromise(fetch(uri, requestOptions(data, 'same-origin', 'POST', csrf))).then(response => {
...
function setPasswordGuest(password: string, guestAccountRegistrationToken: string, csrf: CsrfState): Promise<boolean> {
  const data = {
    password,
    guestAccountRegistrationToken
  };
  return logPromise(fetch(`${routes.contributionsSetPasswordGuest}`, requestOptions(data, 'same-origin', 'PUT', csrf))).then(response => {
...
export { postRegularPaymentRequest, regularPaymentFieldsFromAuthorisation, PaymentSuccess, setPasswordGuest };