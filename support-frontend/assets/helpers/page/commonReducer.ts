...
import { Campaign, ReferrerAcquisitionData } from "helpers/tracking/acquisitions";
...
function createCommonReducer(initialState: CommonState): (state?: CommonState, action: Action) => CommonState {
  return function commonReducer(state: CommonState = initialState, action: Action): CommonState {
    switch (action.type) {
      case 'SET_COUNTRY_INTERNATIONALISATION':
        return { ...state,
          internationalisation: { ...state.internationalisation,
      ...
      case 'SET_CURRENCY_ID':
        return { ...state,
          internationalisation: { ...state.internationalisation,
            currencyId: action.useLocalCurrency && state.internationalisation.localCurrencyCountry ? state.internationalisation.localCurrencyCountry.currency : state.internationalisation.defaultCurrency
          }
        };
      ...
      case 'SET_USE_LOCAL_AMOUNTS':
        return { ...state,
          amounts: action.useLocalAmounts && state.internationalisation.localCurrencyCountry ? state.internationalisation.localCurrencyCountry.amounts : state.defaultAmounts
        };
