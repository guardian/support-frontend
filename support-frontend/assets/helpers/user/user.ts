...
const isPostDeployUser = (): boolean => cookie.get('_post_deploy_user') === 'true';
...
// the user cookie.
const getEmailValidatedFromUserCookie = (guuCookie: string | null | undefined) => {
  if (guuCookie) {
...
const init = (dispatch: (...args: Array<any>) => any, actions: UserSetStateActions = defaultUserActionFunctions) => {
  const {
    setId,
    setDisplayName,
    setFirstName,
...
// ----- Exports ----- //
export { init, getUser, isTestUser, isPostDeployUser, signOut, doesUserAppearToBeSignedIn, getEmailValidatedFromUserCookie };