...
const parseHighlights = (highlightString: string): number[][] => highlightString === '' ? [] : highlightString.split(',').map(occurrence => occurrence.split('-').map(num => parseInt(num, 10))) // Somewhat confusingly Loqate's highlight notation is exclusive of the end character
