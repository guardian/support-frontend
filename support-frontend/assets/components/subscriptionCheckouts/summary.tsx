...
const DataList = (props: {
  dataList: DataListItem[];
    ...
    {props.dataList.length > 0 && <dl className={styles.data}>
        {props.dataList.map(item => [<dt>{item.title}</dt>, <dd>{item.value}</dd>])}
      </dl>}
  </div>;
...
const PromotionDiscount = (props: {
  promotion: Promotion | null | undefined;
    ...
    {props.promotion && hasDiscount(props.promotion) && <div className={styles.promo}>
...
const DropDownButton = (props: {
  ...
  showDropDown: boolean;
}) => <button aria-hidden="true" className={styles.dropDown} onClick={props.onClick}>
...
const TabletAndDesktop = (props: PropTypes) => {
  ...
  return <span className={styles.tabletAndDesktop}>
      ...
      <div className={styles.content}>
        ...
        {props.description && <h4 className={styles.titleDescription}>{props.description}</h4>}
        <div>
          <PriceLabel className={styles.pricing} productPrice={props.productPrice} billingPeriod={props.billingPeriod} />
          <PromotionDiscount promotion={getAppliedPromo(props.productPrice.promotions)} />
          {props.dataList && <DataList dataList={props.dataList} />}
        </div>
        {props.changeSubscription ? <ChangeSubscription route={props.changeSubscription} /> : null}
      </div>
    </span>;
};
...
const HideDropDown = (props: {
  ...
  paper: boolean;
    ...
    <div>
      <PriceLabel className={styles.pricing} productPrice={props.productPrice} billingPeriod={props.billingPeriod} />
      {props.paper ? <span className={styles.pricing}>
          &nbsp;&ndash; Voucher booklet
        </span> : null}
    </div>
  </div>;

const ShowDropDown = (props: PropTypes & {
  ...
  title: string;
    ...
    <div className={styles.contentShowDetails}>
      ...
      <PriceLabel className={styles.data} productPrice={props.productPrice} billingPeriod={props.billingPeriod} giftStyles={styles.gift} />
    </div>
    {props.deliveryMethod ? <div className={styles.contentShowDetails}>
...
const Mobile = props => <span className={styles.mobileOnly}>
...
export default class Summary extends Component<PropTypes, StateTypes> {
  ...
  render() {
    ...
    return <aside className={styles.root}>
        ...
        <Mobile onClick={this.toggleDetails} showDropDown={this.state.showDropDown} deliveryMethod={this.props.dataList.length ? this.getDeliveryMethod() : null} paper={product.toLowerCase().includes('paper')} {...this.props} />
      </aside>;
  }
