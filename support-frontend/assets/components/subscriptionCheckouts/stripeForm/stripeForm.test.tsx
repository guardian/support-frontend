...
async function fillOutForm() {
  ...
  await act(async () => fireEvent.change(cardNumber, {
    target: {
      value: '4242424242424242'
    }
  }));
  await act(async () => fireEvent.change(expiry, {
    target: {
      value: '0230'
    }
  }));
  await act(async () => fireEvent.change(cvc, {
    target: {
      value: '123'
    }
  }));
...
describe('Stripe Form', () => {
  ...
  describe('Form submission - empty fields', () => {
    it('shows error messages to the user when the inputs are empty', async () => {
      const button = await screen.findByRole('button');
      await act(async () => fireEvent.click(button));
      expect(await screen.findByText('Please enter a card number', {
        selector: 'span'
      })).toBeInTheDocument();
      expect(await screen.findByText('Please enter an expiry date', {
        selector: 'span'
      })).toBeInTheDocument();
      expect(await screen.findByText('Please enter a CVC number', {
        selector: 'span'
      })).toBeInTheDocument();
    });
