...
const StripeForm = (props: StripeFormPropTypes) => {
  ...
  const handleStripeError = (errorData: any): void => {
    ...
    if (errorData.type === 'validation_error') {
      ...
      setCardErrors(prevData => [...prevData, {
        field: 'cardNumber',
  ...
  const fetchPaymentIntent = token => fetchJson(routes.stripeSetupIntentRecaptcha, requestOptions({
    token,
    stripePublicKey: props.stripeKey,
    isTestUser: props.isTestUser
  }, 'same-origin', 'POST', props.csrf)).then(result => {
  ...
  const handleCardErrors = () => {
    ...
    const {
      fieldData,
      fieldErrors
    }: CardFieldsValidationOutput = cardFields.reduce((newData: CardFieldsValidationOutput, cardFieldName: CardFieldName) => {
      ...
      return {
        ...
        fieldData: { ...newData.fieldData,
          [cardFieldName]: { ...existingFieldData,
            ...(error ? {
              error
            } : {})
          }
  ...
  const handleChange = event => {
    if (cardFieldsData[event.elementType].error) {
      setCardFieldsData(prevData => ({ ...prevData,
        [event.elementType]: { ...prevData[event.elementType],
          error: ''
        }
      }));
    } else {
  ...
  const checkRecaptcha = () => {
    if (window.guardian.recaptchaEnabled && !recaptchaCompleted && recaptchaError === null) {
      setRecaptchaError({
        field: 'recaptcha',
  ...
  const handleCardSetupAndPay = () => handleCardSetup(setupIntentClientSecret).then(props.setStripePaymentMethod).then(() => {
   ...
   * Rendering
   */
  const errors = [...props.allErrors, ...(cardErrors || []), ...(recaptchaError ? [recaptchaError] : [])];
  return <span>
      {stripe && <fieldset css={marginTop}>
        <CardNumberWithError id="card-number" error={cardFieldsData.cardNumber.error} label="Card number" options={{
        style: {
          base: { ...baseStyles
          },
        ...
        <CardExpiryWithError id="card-expiry" error={cardFieldsData.cardExpiry.error} label="Expiry date" options={{
        style: {
          base: { ...baseStyles
          },
          invalid: { ...invalidStyles
        ...
        <CardCvcWithError id="cvc" error={cardFieldsData.cardCvc.error} label="CVC" options={{
        style: {
          base: { ...baseStyles
          },
          invalid: { ...invalidStyles
        ...
        {window.guardian.recaptchaEnabled ? <RecaptchaWithError id="robot_checkbox" label="Security check" error={recaptchaError && recaptchaError.message} /> : null}
        <div className="component-stripe-submit-button">
          <ThemeProvider theme={buttonReaderRevenue}>
            <Button id="qa-stripe-submit-button" onClick={requestSCAPaymentMethod} priority="primary" icon={<SvgArrowRightStraight />} iconSide="right" disabled={disableButton}>
              {props.buttonText}
            </Button>
          </ThemeProvider>
        </div>
