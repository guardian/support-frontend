...
export type PostcodeFinderActions = (Scoped<AddressType> & {
...
const postcodeFinderActionCreatorsFor = (scope: AddressType) => ({
  ...
  fetchResults: (postcode: Option<string>) => (dispatch: Dispatch<PostcodeFinderActions>) => {
    if (!postcode) {
      ...
      getAddressesForPostcode(postcode).then(results => {
        dispatch({
          type: 'FILL_POSTCODE_FINDER_RESULTS',
          results,
          scope
...
const postcodeFinderReducerFor = (scope: AddressType) => (state: PostcodeFinderState = initialState, action: PostcodeFinderActions): PostcodeFinderState => {
