...
import { formError, nonEmptyString, notNull, removeError, validate, nonSillyCharacters } from "helpers/subscriptionsForms/validation";
...
export type Action = (Scoped<AddressType> & {
  ...
  type: "SET_STATE";
  state: string;
}) | SetCountryChangedAction | (Scoped<AddressType> & {
...
// ----- Functions ----- //
const isPostcodeOptional = (country: Option<IsoCountry>): boolean => country !== 'GB' && country !== 'AU' && country !== 'US' && country !== 'CA';

const checkLength = (input: string | null, maxLength: number): boolean => input == null || input.length <= maxLength;

const isStateNullable = (country: Option<IsoCountry>): boolean => country !== 'AU' && country !== 'US' && country !== 'CA';

export const isHomeDeliveryInM25 = (fulfilmentOption: Option<FulfilmentOptions>, postcode: Option<string>, allowedPrefixes: string[] = M25_POSTCODE_PREFIXES) => {
...
const applyBillingAddressRules = (fields: FormFields, addressType: AddressType): FormError<FormField>[] => validate([{
  ...
  error: formError('lineOne', 'Please use only letters, numbers and punctuation.')
}, {
  rule: nonSillyCharacters(fields.lineTwo),
  error: formError('lineTwo', 'Please use only letters, numbers and punctuation.')
}, {
  rule: nonEmptyString(fields.city),
  error: formError('city', `Please enter a ${addressType} city.`)
}, {
  rule: nonSillyCharacters(fields.city),
  error: formError('city', 'Please use only letters, numbers and punctuation.')
}, {
  rule: isPostcodeOptional(fields.country) || nonEmptyString(fields.postCode) && nonSillyCharacters(fields.postCode),
  error: formError('postCode', `Please enter a ${addressType} postcode.`)
}, {
  rule: nonSillyCharacters(fields.postCode),
  error: formError('postCode', 'Please use only letters, numbers and punctuation.')
}, {
  ...
  error: formError('state', fields.country === 'CA' ? `Please select a ${addressType} province/territory.` : `Please select a ${addressType} state.`)
}, {
  rule: checkLength(fields.state, 40),
  error: formError('state', fields.country === 'CA' ? `Please enter a ${addressType} province/territory no longer than 40 characters` : `Please enter a ${addressType} state name no longer than 40 characters`)
}]);

const applyDeliveryAddressRules = (fulfilmentOption: Option<FulfilmentOptions>, fields: FormFields, addressType: AddressType): FormError<FormField>[] => {
...
const addressActionCreatorsFor = (scope: AddressType) => ({
  ...
  setAddressLineOne: (lineOne: string): (...args: Array<any>) => any => setFormSubmissionDependentValue(() => ({
    scope,
    type: 'SET_ADDRESS_LINE_1',
    lineOne
  })),
  ...
  setTownCity: (city: string): (...args: Array<any>) => any => setFormSubmissionDependentValue(() => ({
    scope,
    type: 'SET_TOWN_CITY',
    city
  })),
  setState: (state: string): (...args: Array<any>) => any => setFormSubmissionDependentValue(() => ({
    type: 'SET_STATE',
    state,
    scope
  })),
  setPostcode: (postCode: string): (...args: Array<any>) => any => setFormSubmissionDependentValue(() => ({
    type: 'SET_POSTCODE',
    postCode,
    scope
  }))
...
// ----- Export ----- //
export { addressReducerFor, getFormFields, getStateFormErrors, getPostcodeForm, setFormErrorsFor, addressActionCreatorsFor, isPostcodeOptional, applyBillingAddressRules, applyDeliveryAddressRules, isStateNullable };