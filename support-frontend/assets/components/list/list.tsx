...
function ListWith(ListItemComponent: React.ComponentType<ListItemProps>) {
  function ListComponent(props: ListProps) {
    return <ul css={[list, props.cssOverrides]}>
        {props.items.map(item => <ListItemComponent key={item.content} item={item} colour={props.bulletColour || 'light'} size={props.bulletSize || 'large'} />)}
      </ul>;
  }
