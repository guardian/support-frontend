...
const billingPeriodQuantifier = (numberOfBillingPeriods: number, noun: string) => numberOfBillingPeriods > 1 ? `/${noun} for ${numberOfBillingPeriods} ${noun}s` : ` for 1 ${noun}`;

function getSummary(glyph: string, price: number, discountedPrice: number, numberOfDiscountedPeriods: number | null | undefined, billingPeriod: BillingPeriod) {
...
function discountSummary(price: number, promoCode: string, discountedPrice: number, discountAmount: number, numberOfDiscountedPeriods: number | null | undefined, billingPeriod: BillingPeriod, country: IsoCountry) {
  const countryGroup = countryGroups[fromCountry(country) || GBPCountries];
  const {
    glyph
  } = currencies[countryGroup.currency];
  return <div className="component-promotion-summary">
...
function PromotionSummary(props: PropTypes) {
  if (props.country) {
    ...
    if (promotion && promotion.discountedPrice && promotion.discount) {
      return discountSummary(productPrice.price, promotion.promoCode, promotion.discountedPrice, promotion.discount.amount, promotion.numberOfDiscountedPeriods, props.billingPeriod, country);
    }
  }
