...
function Tabs({
  ...
  return <div>
      <div css={tabList} role="tablist" aria-label={tabsLabel}>
        ...
        return <TabControllerElement css={tabButton} role="tab" id={tab.id} href={tab.href} aria-selected={selected} aria-controls={`${tab.id}-tab`} onClick={event => {
      ...
      {tabs.map((tab: TabProps) => <div css={tabPanel} role="tabpanel" id={`${tab.id}-tab`} aria-labelledby={tab.id} hidden={!tab.selected}>
          {tab.content}
        </div>)}
    </div>;
}
