...
function mapDispatchToProps(dispatch: Dispatch<Action>) {
  return {
    onClick: (email: string, csrf: CsrfState) => {
      ...
      sendMarketingPreferencesToIdentity(true, // it's TRUE because the button says Sign Me Up!
      email, dispatch, csrf, 'MARKETING_CONSENT');
    }
  };
}
...
function MarketingButton(props: ButtonPropTypes) {
  ...
  if (confirmedOrPending) {
    return <ThemeProvider theme={buttonBrandAlt}>
        <Button css={marginForButton} iconSide="right" icon={<SvgEnvelope />} disabled>
  ...
  return <ThemeProvider theme={buttonBrandAlt}>
      <Button css={marginForButton} iconSide="right" aria-label="Sign me up to news and offers from the Guardian" onClick={() => props.onClick(props.email, props.csrf)} icon={<SvgEnvelope />}>
        Sign me up
      </Button>
    </ThemeProvider>;
}
...
function MarketingConsent(props: PropTypes) {
  ...
  if (checkEmail(props.email)) {
    return <section css={maxWidth}>
        <h3 css={subHeading}>Contributions, subscriptions and membership</h3>
        <p css={sansText}>
          Get related news and offers &ndash; whether you are a contributor, subscriber, member or
        ...
        <p css={tinyText}>
          {props.confirmOptIn === true ? 'We\'ll be in touch. Check your inbox for a confirmation link' : 'You can unsubscribe at any time'}
        </p>
        <p css={sansText}>
