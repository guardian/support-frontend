...
function sendMarketingPreferencesToIdentity(optIn: boolean, email: string, dispatch: (...args: Array<any>) => any, csrf: CsrfState, scope: string): void {
  const {
  ...
  fetch(`${routes.contributionsSendMarketing}`, requestData(email, csrf)).then(response => {
