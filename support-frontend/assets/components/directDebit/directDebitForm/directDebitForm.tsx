...
import type { Action, Phase, SortCodeIndex } from "components/directDebit/directDebitActions";
import { closeDirectDebitGuarantee, confirmDirectDebitClicked, openDirectDebitGuarantee, payDirectDebitClicked, setDirectDebitFormPhase, updateAccountHolderConfirmation, updateAccountHolderName, updateAccountNumber, updateSortCode } from "components/directDebit/directDebitActions";
...
type PropTypes = {
  ...
  updateSortCode: (index: SortCodeIndex, event: React.SyntheticEvent<HTMLInputElement>) => void;
  updateAccountNumber: (accountNumber: React.SyntheticEvent<HTMLInputElement>) => void;
  updateAccountHolderName: (accountHolderName: React.SyntheticEvent<HTMLInputElement>) => void;
  updateAccountHolderConfirmation: (accountHolderConfirmation: React.SyntheticEvent<HTMLInputElement>) => void;
  ...
  confirmDirectDebitClicked: (onPaymentAuthorisation: (arg0: PaymentAuthorisation) => void) => void;
...
function mapDispatchToProps(dispatch: Dispatch<Action>) {
  return {
    ...
    confirmDirectDebitClicked: (onPaymentAuthorisation: (arg0: PaymentAuthorisation) => void) => {
    ...
    updateSortCode: (index: SortCodeIndex, event: React.SyntheticEvent<HTMLInputElement>) => {
    ...
    updateAccountHolderName: (event: React.SyntheticEvent<HTMLInputElement>) => {
      const accountHolderName: string = event.target.value;
      dispatch(updateAccountHolderName(accountHolderName));
    },
    updateAccountHolderConfirmation: (event: React.SyntheticEvent<HTMLInputElement>) => {
...
// ----- Component ----- //
const DirectDebitForm = (props: PropTypes) => <div className="component-direct-debit-form">

    <AccountHolderNameInput phase={props.phase} onChange={props.updateAccountHolderName} value={props.accountHolderName} />

    <AccountNumberInput phase={props.phase} onChange={props.updateAccountNumber} value={props.accountNumber} />

    <SortCodeInput phase={props.phase} onChange={props.updateSortCode} sortCodeArray={props.sortCodeArray} />

    <ConfirmationInput phase={props.phase} onChange={props.updateAccountHolderConfirmation} checked={props.accountHolderConfirmation} />

    <PaymentButton buttonText={props.buttonText} phase={props.phase} onPayClick={() => props.payDirectDebitClicked()} onEditClick={() => props.editDirectDebitClicked()} onConfirmClick={() => props.confirmDirectDebitClicked(props.onPaymentAuthorisation)} />

    <ErrorMessage message={props.formError} svg={<SvgExclamationAlternate />} />

    <LegalNotice countryGroupId={props.countryGroupId} />

    <DirectDebitGuarantee isDDGuaranteeOpen={props.isDDGuaranteeOpen} openDDGuaranteeClicked={props.openDDGuaranteeClicked} closeDDGuaranteeClicked={props.closeDDGuaranteeClicked} />
  </div>;
...
function AccountNumberInput(props: {
  ...
  const editable = <input id="account-number-input" value={props.value} onChange={props.onChange} pattern="[0-9]*" minLength="6" maxLength="10" className="component-direct-debit-form__text-field focus-target" />;
  const locked = <span>
      {props.value}
    </span>;
  return <div className="component-direct-debit-form__account-number">
...
function AccountHolderNameInput(props: {
  ...
  const editable = <input id="account-holder-name-input" value={props.value} onChange={props.onChange} maxLength="40" className="component-direct-debit-form__text-field focus-target" />;
  const locked = <span>
      {props.value}
    </span>;
  return <div className="component-direct-debit-form__account-holder-name">
...
function ConfirmationInput(props: {
  ...
  const editable = <span>
      <div className="component-direct-debit-form__confirmation-css-checkbox">
        <input className="component-direct-debit-form__confirmation-input" id="confirmation-input" type="checkbox" onChange={props.onChange} checked={props.checked} />
        <label id="qa-confirmation-input" className="component-direct-debit-form__confirmation-label" htmlFor="confirmation-input" />
      </div>
  ...
  return <div className="component-direct-debit-form__account-holder-confirmation">
      <div>
...
function PaymentButton(props: {
  ...
  if (props.phase === 'entry') {
    return <button id="qa-submit-button-1" className="component-direct-debit-form__cta component-direct-debit-form__cta--pay-button focus-target" onClick={props.onPayClick}>
  ...
  if (props.phase === 'confirmation') {
    return <span>
        <button className="component-direct-debit-form__cta component-direct-debit-form__cta--edit-button focus-target" onClick={props.onEditClick}>
          <SvgArrowRightStraight />
          <span className="component-direct-debit-form__cta-text component-direct-debit-form__cta-text--inverse">Back</span>
        </button>
        <button id="qa-submit-button-2" className="component-direct-debit-form__cta component-direct-debit-form__cta--confirm-button focus-target" onClick={props.onConfirmClick}>
...
function LegalNotice(props: {
  ...
  return <div className="component-direct-debit-form__legal-notice">
      <p>
