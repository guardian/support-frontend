...
import { payDirectDebitClicked, setDirectDebitFormPhase, updateAccountHolderName, updateAccountNumber, updateSortCodeString, updateAccountHolderConfirmation } from "components/directDebit/directDebitActions";
...
const fieldNames = ['accountHolderName', 'sortCodeString', 'accountNumber', 'accountHolderConfirmation']; // ----- Component ----- //
...
class DirectDebitForm extends Component<PropTypes, StateTypes> {
  constructor(props: PropTypes) {
    ...
    this.state = {
      accountHolderName: {
        ...
        rule: accountHolderName => accountHolderName.match(/^\D+$/) && nonSillyCharacters(accountHolderName)
      },
      sortCodeString: {
        error: '',
  ...
  render() {
    ...
    const showGeneralError = props.allErrors.length === 0 && accountErrorsLength === 0 && (props.submissionError !== null || props.formError !== null && props.formError.length > 0);
    return <span>
        {props.phase === 'entry' && <Form {...props} showGeneralError={showGeneralError} accountErrors={this.state.allErrors} accountErrorsLength={accountErrorsLength} accountHolderNameError={state.accountHolderName.error} accountNumberError={state.accountNumber.error} sortCodeError={state.sortCodeString.error} accountHolderConfirmationError={state.accountHolderConfirmation.error} onChange={this.onChange} onSubmit={this.handleErrorsAndCheckAccount} />}
        {props.phase === 'confirmation' && <Playback editDirectDebitClicked={props.editDirectDebitClicked} onSubmit={this.onSubmit} accountHolderName={props.accountHolderName} accountNumber={props.accountNumber} sortCodeString={props.sortCodeString} buttonText={props.buttonText} allErrors={props.allErrors} />}
      </span>;
  }
