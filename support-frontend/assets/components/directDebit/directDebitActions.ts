...
function payDirectDebitClicked(): (...args: Array<any>) => any {
  return (dispatch: (...args: Array<any>) => any, getState: (...args: Array<any>) => any) => {
    const {
      sortCodeString,
      sortCodeArray,
      accountNumber,
    ...
    checkAccount(sortCode, accountNumber, isTestUser, csrf).then(response => {
      if (!response.ok) {
      ...
      switch (e.message) {
        case 'invalid_input':
          msg = 'Your bank details are invalid. Please check them and try again';
          break;

        case 'incorrect_input':
          msg = 'Your bank details are not correct. Please check them and try again';
          break;
...
function confirmDirectDebitClicked(onPaymentAuthorisation: (arg0: PaymentAuthorisation) => void): (...args: Array<any>) => any {
  return (dispatch: (...args: Array<any>) => any, getState: (...args: Array<any>) => any) => {
    const {
      sortCodeString,
      sortCodeArray,
...
// ----- Exports ----//
export { openDirectDebitPopUp, closeDirectDebitPopUp, openDirectDebitGuarantee, closeDirectDebitGuarantee, updateSortCode, updateSortCodeString, updateAccountNumber, updateAccountHolderName, updateAccountHolderConfirmation, setDirectDebitFormError, resetDirectDebitFormError, payDirectDebitClicked, confirmDirectDebitClicked, setDirectDebitFormPhase };