...
import { CountryGroup } from "helpers/internationalisation/countryGroup";
...
// ----- Render ----- //
const renderCountryGroup = (selectedCountryGroup: CountryGroup) => (countryGroup: CountryGroup) => <li className="countryGroups__item">
    <a href={`/${countryGroup.supportInternationalisationId}/contribute`}>
      ...
      {countryGroup === selectedCountryGroup ? <span className="icon">
          <SvgCheckmark />
        </span> : null}
    </a>
  </li>;
...
function RoundelHeader(props: PropTypes) {
  return <header role="banner" className="gu-content__header">
      <a className="glogo" href="https://www.theguardian.com">
        <SvgGuardianLogo />
        <h2 className="glogo-alt-text">Return to The Guardian</h2>
      </a>
      {props.selectedCountryGroup ? <details className="countryGroups">
          ...
          <ul className="countryGroups__list">
            {Object.keys(countryGroups).map(countryGroup => countryGroups[countryGroup]).map(renderCountryGroup((props.selectedCountryGroup as any)))}
          </ul>
        </details> : null}
    </header>;
}
