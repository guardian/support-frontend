...
function internationalisationID(countryGroupId: CountryGroupId | null | undefined = null): string | null | undefined {
...
const Links = ({
  ...
  return <nav className={classNameWithModifiers('component-header-links', [location])}>
      <ul className="component-header-links__ul" ref={getRef}>
        ...
        opensInNewWindow,
        additionalClasses
      }) => <li className={cx(classNameWithModifiers('component-header-links__li', [urlWithoutParams.endsWith(href) ? 'active' : null]), additionalClasses)}>
          <a onClick={sendTrackingEventsOnClick({
          id: ['header-link', trackAs, location].join(' - '),
          componentType: 'ACQUISITIONS_OTHER'
        })} className="component-header-links__link" href={href} target={opensInNewWindow ? '_blank' : null}>
            {text}
          </a>
        </li>)}
      </ul>
