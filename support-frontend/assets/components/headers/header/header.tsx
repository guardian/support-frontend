...
import { ElementResizer } from "helpers/polyfills/layout";
...
const getMenuStateMetrics = ({
  ...
  const [logoLeft, menuWidth, containerLeft, containerWidth] = [logoRef.getBoundingClientRect().left, menuRef.getBoundingClientRect().width, containerRef.getBoundingClientRect().left, containerRef.getBoundingClientRect().width];
...
const TopNav = ({
  ...
  getLogoRef,
  utility
}: TopNavPropTypes) => <div className="component-header-topnav">
    <div className="component-header-topnav__utility">{utility}</div>
    {display === 'checkout' && <div className="component-header-topnav__checkout">
        <div />
...
export default class Header extends Component<PropTypes, State> {
  ...
  componentDidMount() {
    if (this.props.display === 'navigation' && this.menuRef && this.logoRef && this.containerRef) {
      this.observer = onElementResize([this.logoRef, this.menuRef, this.containerRef], ([logoRef, menuRef, containerRef]) => {
        this.setState(getMenuStateMetrics({
          menuRef,
          logoRef,
          containerRef
        }));
  ...
  render() {
    ...
    return <header className={classNameWithModifiers('component-header', [fitsLinksInOneRow ? 'one-row' : null, display === 'navigation' ? 'display-navigation' : null, !fitsLinksAtAll ? 'display-veggie-burger' : null, display === 'checkout' ? 'display-checkout' : null])}>
        {!!isTestUser && <div className="test-user-banner">
            <span>You are a test user</span>
          </div>}
        <div className="component-header__wrapper" ref={el => {
        this.containerRef = el;
      }}>
          <div className="component-header__row">
            <TopNav display={display} utility={display === 'navigation' && fitsLinksAtAll ? utility : null} getLogoRef={el => {
            this.logoRef = el;
          }} />
            {display === 'navigation' && <MobileMenuToggler links={<Links countryGroupId={countryGroupId} location="mobile" />} countryGroupId={countryGroupId} utility={utility} />}
          </div>
          {display === 'navigation' && <div className="component-header__row">
              <Links countryGroupId={countryGroupId} location="desktop" getRef={el => {
            this.menuRef = el;
          }} />
            </div>}
          {display === 'checkout' && <div className="component-header__row component-header-checkout--row">
              <div className="component-header--padlock"><Padlock /></div>
              <div>Checkout</div>
            </div>}
        </div>
