...
const HeroWrapper = ({
  ...
  appearance,
  className
}: WrapperPropTypes) => <div className={`${className || ''} ${classNameWithModifiers('component-product-page-hero', [...modifierClasses, appearance])}`}>
    <LeftMarginSection>
      {children}
    </LeftMarginSection>
  </div>;
...
const ProductPageHeroHeader = ({
  ...
  orderIsAGift,
  giftImage
}: ProductPageHeroHeaderTypes) => <div>
    <HeroHeading {...{
    hasCampaign,
    orderIsAGift,
    giftImage
  }}>
      <HeadingBlock overheading={orderIsAGift ? 'The Guardian Weekly gift subscription' : overheading} orderIsAGift={orderIsAGift}>
        {heading}
      </HeadingBlock>
    </HeroHeading>
  </div>;
