...
import type { PayPalCheckoutDetails, PayPalUserDetails } from "helpers/forms/paymentIntegrations/payPalRecurringCheckout";
...
function mapStateToProps(state: CheckoutState, ownProps) {
  return {
    ...
    amount: finalPrice(state.page.checkout.productPrices, state.common.internationalisation.countryId, ownProps.billingPeriod).price,
...
function PayPalOneClickCheckoutButton(props: PropTypes) {
  ...
  return <div css={payPalButton}>
      {props.wasClicked && <GeneralErrorMessage errorReason={errorReason} errorHeading={submissionErrorHeading} />}
      <PayPalExpressButton onPayPalCheckoutCompleted={props.onPayPalCheckoutCompleted} csrf={props.csrf} currencyId={props.currencyId} hasLoaded={props.hasLoaded} canOpen={() => true} onClick={() => props.onClick(props.billingPeriod, props.trackingId, props.product)} formClassName="form--contribution" isTestUser={props.isTestUser} setupRecurringPayPalPayment={props.setupRecurringPayPalPayment} amount={props.amount} billingPeriod={props.billingPeriod} />
    </div>;
}
