...
// ---- Helpers ----- //
const getInitialTickerValues = (tickerCountType: TickerCountType): Promise<DataFromServer> => fetch(tickerCountType === 'money' ? '/ticker.json' : '/supporters-ticker.json').then(resp => resp.json()).then(data => {
...
export default class ContributionTicker extends Component<PropTypes, StateTypes> {
  ...
  increaseTextCounter = (dataFromServer: DataFromServer) => () => {
    ...
    const finishedCounting = nextCount <= this.state.count || // count isn't going up because total is too small
  ...
  render() {
    ...
    const classModifiers = [this.props.tickerEndType, this.state.goalReached ? 'goal-reached' : '', !readyToRender ? 'hidden' : ''];
    ...
    return <div className={wrapperClassName}>
        ...
        <div className="contributions-landing-ticker__progress-bar">
          <div className="contributions-landing-ticker__progress">
            <div className="contributions-landing-ticker__filled-progress" style={{
            transform: progressBarAnimation
          }} />
          </div>
          <div className="contributions-landing-ticker__marker" style={{
