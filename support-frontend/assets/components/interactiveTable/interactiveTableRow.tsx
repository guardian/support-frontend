...
export function InteractiveTableHeaderRow({
  ...
  return <tr role="row" css={[tableRow, tableHeaderRow]}>
      {columns.map((col, index) => <th scope="col" role="columnHeader" css={col.isStyleless ? '' : [tableCell, headingCell, ...(col.isIcon ? [iconCell, css`
...
export function InteractiveTableFooterRow({
  ...
  return <tr role="row" css={[finalRow, yellowBackground]}>
...
export function InteractiveTableRow({
  ...
  return <tr role="row" css={[tableRow, tableContentRow, showDetails ? tableRowOpen : '']}>
      {columns.map((column, index) => {
      if (column.isPrimary) {
        return <th scope="row" role="rowHeader" css={[tableCell, primaryTableCell]}>
              {column.content}
            </th>;
      }

      return <td role="cell" css={[tableCell, iconCell, css`
              -ms-grid-column: ${index + 1};
            `]}>{column.content}
          </td>;
    })}
      <td role="cell" css={expandableButtonCell}>
        <Button hideLabel priority="subdued" icon={<SvgChevronDownSingle />} css={[toggleButton, showDetails ? toggleButtonOpen : '']} aria-expanded={showDetails ? 'true' : 'false'} aria-controls={`${rowId}-details`} onClick={onRowClick}>
          {`${showDetails ? 'Hide' : 'Show'} more details`}
        </Button>
      </td>
      <td role="cell" hidden={!showDetails} id={`${rowId}-details`} css={[tableCell, detailsCell, showDetails ? detailsVisible : detailsHidden]}>
        {details}
      </td>
    </tr>;
}
