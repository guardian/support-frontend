...
exec(tscCmd, (error, stdout) => {
	...
	const typescriptErrors = lines.reduce((accumulator, line) => {
		const capturedGroups =
			/^(?<errorPath>[^(]*(\.tsx|\.ts)).*?(?<errorCode>TS\d+):\s(?<errorMessage>.*)$/gm.exec(
				line,
			);
  ...
  const fileContents = {
	...
	let totalErrorCount = Object.keys(typescriptErrors).reduce(
	...
	console.log(
		`${totalErrorCount} Typescript errors have been logged and saved in ${errorFileName}.`,
	);
	...
	const filesWithErrors = Object.keys(typescriptErrors)
		.map((errorCode) => {
			return typescriptErrors[errorCode].map(
				(errorInstance) => errorInstance.path,
			);
		})
		.flat()
