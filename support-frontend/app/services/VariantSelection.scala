...
object VariantSelection extends StrictLogging {
  ...
  /** Main entry point for variant selection.
    ...
    * @return
    *   A tuple of (selected variant, effective test name for tracking), or None if selection fails
    */
  def selectVariant(
    ...
    val variantOption = methodology match {
      case _: ABTest =>
    ...
    variantOption.map(variant => (variant, effectiveTestName))
  }
  ...
  /** Standard AB test variant selection using MVT ID.
    ...
    */
  def selectVariantUsingMVT(test: LandingPageTest, mvtId: Int): Option[LandingPageVariant] = {
  ...
  /** Select a random variant using non-deterministic randomness.
    ...
    */
  private def selectRandomVariant(test: LandingPageTest): Option[LandingPageVariant] = {
    if (test.variants.isEmpty) {
      logger.warn(s"Test ${test.name} has no variants")
      None
    } else {
