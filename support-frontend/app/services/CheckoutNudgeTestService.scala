...
import admin.settings.CheckoutNudgeTest
...
import services.CheckoutNudgeTestService.parseCheckoutNudgeTest
...
object CheckoutNudgeTestService {
  ...
  def parseCheckoutNudgeTest(record: java.util.Map[String, AttributeValue]): Either[io.circe.Error, CheckoutNudgeTest] =
    dynamoMapToJson(record).as[CheckoutNudgeTest]
}

/** A service for polling DynamoDb for checkout nudge tests config
  */
class CheckoutNudgeTestServiceImpl(stage: Stage)(implicit val ec: ExecutionContext, system: ActorSystem)
    extends CheckoutNudgeTestService
    with StrictLogging {
  private val cachedTests = new AtomicReference[List[CheckoutNudgeTest]](Nil)
  ...
  private def fetchCheckoutNudgeTests(): Future[List[CheckoutNudgeTest]] = {
    val condition = Condition
      .builder()
      .comparisonOperator(ComparisonOperator.EQ)
      .attributeValueList(AttributeValue.builder().s("CheckoutNudge").build())
      .build()
    ...
    dynamoClient
      ...
      .map { result =>
        ...
        items
          .map(parseCheckoutNudgeTest)
          .map({
            case err @ Left(msg) =>
              AwsCloudWatchMetricPut(cloudwatchClient)(getCheckoutNudgeTestsError(stage))
              logger.error(s"Could not decode checkout nudge test config: $msg")
              err
            case ok @ Right(_) => ok
          })
          .collect({ case Right(test) => test }) // filter out any invalid tests
          .filter(test => test.status == admin.settings.Status.Live) // Only return Live tests
      }
  }

  // Start polling DynamoDb
  system.scheduler.scheduleAtFixedRate(0.minute, 1.minute)(() => {
    fetchCheckoutNudgeTests()
      .map { tests =>
        cachedTests.set(tests)
      }
      .recover { case NonFatal(error) =>
        AwsCloudWatchMetricPut(cloudwatchClient)(getCheckoutNudgeTestsError(stage))
        logger.error(s"Error fetching checkout nudge tests from dynamodb: ${error.getMessage}")
        Nil
      }
  })

  def getTests(): List[CheckoutNudgeTest] = cachedTests.get()
}
