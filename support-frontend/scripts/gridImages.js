...
const imageNames = Object.keys(gridFiles).map(async fileName => ({
  fileName,
  presentIn: await fileSearch(['./assets'], fileName, { recursive: true, fileMask: 'jsx' }),
}));
...
(async () => {
  ...
  const gridFilesDuplicate = { ...gridFiles };
  keysToDelete.forEach((key) => {
    delete gridFilesDuplicate[key];
  });

  console.log(gridFilesDuplicate);
})();
