...
const cssLoaders = [{
  ...
  options: {
    ...
    includePaths: [path.resolve(__dirname, 'assets'), path.resolve(__dirname)]
  }
...
module.exports = (cssFilename, outputFilename, minimizeCss) => ({
  ...
  module: {
    rules: [{
      ...
      exclude: [{
        test: /node_modules/,
        exclude: [/@guardian\/(?!(automat-modules))/]
      }],
      loader: 'babel-loader'
    }, {
      ...
      use: [MiniCssExtractPlugin.loader, {
        ...
        options: {
          modules: true,
          getLocalIdent: (context, localIdentName, localName) => getClassName(path.relative(__dirname, context.resourcePath), localName)
        }
      }, ...cssLoaders]
    }, {
      test: /\.css$/,
