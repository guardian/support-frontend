...
import com.gu.support.workers.{Annual, BillingPeriod, GiftCode}
...
class GiftCodeGeneratorService {
  ...
  def generateCode(billingPeriod: BillingPeriod): GiftCode = GiftCodeGenerator.randomGiftCodes.next().withDuration(toDuration(billingPeriod))
}
