...
@IntegrationTest
class DynamoServiceSpec extends AsyncFlatSpec with Matchers {
  ...
  "DynamoService" should "work" ignore {
    ...
    info(s"There are ${discounts.length} discounts")
    discounts.filter(_.discount.get.durationMonths.isEmpty).foreach(p => info(s"$p"))
