...
class NextState(state: CreateSalesforceContactState) {
  ...
  def toNextContribution(
    ...
    ), requestId, user, product, analyticsInfo, None, None, state.csrUsername, acquisitionData)
  ...
  def toNextWeekly(
    ...
    ), requestId, user, product, analyticsInfo, firstDeliveryDate, promoCode, state.csrUsername, acquisitionData)
