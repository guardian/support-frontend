...
class ZuoraDigitalSubscriptionGiftPurchaseHandler(
  ...
  def subscribe(state: DigitalSubscriptionGiftPurchaseState, csrUsername: Option[String]): Future[SendThankYouEmailState] =
    for {
      subscriptionBuildResult <- Future.fromTry(digitalSubscriptionGiftPurchaseBuilder.build(state, csrUsername).leftMap(BuildSubscribePromoError).toTry)
