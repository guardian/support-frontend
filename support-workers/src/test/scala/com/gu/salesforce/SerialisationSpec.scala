...
class SerialisationSpec extends AnyFlatSpec with Matchers with LazyLogging {
  ...
  "UpsertData" should "serialise to correct UK json" in {
    newContactUK.asJson.printWith(Printer.noSpaces.copy(dropNullValues = true)) should be(parse(upsertJson).toOption.get.noSpaces)
  }

  "UpsertData" should "serialise to correct US json" in {
    newContactUS.asJson.printWith(Printer.noSpaces.copy(dropNullValues = true)) should be(parse(upsertJsonWithState).toOption.get.noSpaces)
  }
  ...
  "UpsertData" should "serialise to correct json when telephoneNumber provided" in {
    ...
    newContactWithTelephone.asJson.printWith(Printer.noSpaces.copy(dropNullValues = true)) should be(parse(upsertJsonWithTelephoneNumber).toOption.get.noSpaces)
  }

  "UpsertData" should "serialise to correct json when billing address provided" in {
    newContactUKWithBillingAddress.asJson.
      printWith(Printer.noSpaces.copy(dropNullValues = true)) should be(parse(upsertJsonWithBillingAddress).toOption.get.noSpaces)
  }
  ...
  "UpsertData" should "serialise to correct json when billing address and delivery address provided" in {
    newContactUKWithBothAddressesAndTelephone.asJson.
      printWith(Printer.noSpaces.copy(dropNullValues = true)) should be(parse(upsertJsonWithBillingAndDeliveryAddresses).toOption.get.noSpaces)
  }

  "Gift recipient upsert data" should "serialise to correct json" in {
    giftRecipientUpsert.asJson.printWith(Printer.noSpaces.copy(dropNullValues = true)) should be(parse(giftRecipientUpsertJson).toOption.get.noSpaces)
  }
