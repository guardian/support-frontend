  ...
  def start(): Unit = {
    ...
    actorSystem.scheduler.schedule(5.seconds, 60.seconds) {
      client.healthy.onComplete {
        ...
        case Success(false) => SafeLogger.error(scrub"Regular contributions state machine is not available")
        case Failure(exception) => SafeLogger.error(scrub"Regular contributions state machine is not available", exception)
      }
    }
  }
}
