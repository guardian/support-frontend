...
class RegularContributions(
  ...
  private def createContributorWithUser(fullUser: AuthenticatedIdUser)(implicit request: OptionalAuthRequest[CreateRegularContributorRequest]) = {
    ...
    val result = for {
      ...
      response <- client.createContributor(request.body, contributor(user, request.body), request.uuid, AuthenticatedAccess).leftMap(_.toString)
    } yield response
  ...
  private def createContributorAndUser()(implicit request: OptionalAuthRequest[CreateRegularContributorRequest]) = {
    val result = for {
      ...
      response <- client.createContributor(request.body, contributor(user, request.body), request.uuid, SessionAccess(sessionId)).leftMap(_.toString)
