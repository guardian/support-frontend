...
class OneOffContributions(
  ...
  def autofill: Action[AnyContent] = AuthenticatedAction(membersIdentityClientId).async { implicit request =>
  ...
  def displayForm(): Action[AnyContent] = MaybeAuthenticatedAction(membersIdentityClientId).async { implicit request =>
    request.user.fold {
