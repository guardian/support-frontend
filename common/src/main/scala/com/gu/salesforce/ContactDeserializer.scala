...
object ContactDeserializer {
  ...
  import Keys._
  implicit val sfDateReads = jodaDateReads("yyyy-MM-dd'T'HH:mm:ss.SSSZ")

  implicit val contact: Reads[Contact] = (
    (__ \ IDENTITY_ID).read[String] or Reads.pure("") and //FIXME make this an option
      ...
      (__ \ MAILING_COUNTRY).readNullable[String]
    ) (Contact)
