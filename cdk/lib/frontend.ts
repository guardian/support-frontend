...
export class Frontend extends GuStack {
    ...
    /opt/cloudwatch-logs/configure-logs application ${this.stack} ${this.stage} ${app} /var/log/support-frontend/application.log '%Y-%m-%dT%H:%M:%S,%f%z'`);
		...
		if (shouldCreateAlarms) {
			...
			new GuAlarm(this, 'GetCheckoutNudgeTestsError', {
				app,
				alarmName: alarmName('support-frontend GetCheckoutNudgeTestsError'),
				alarmDescription: alarmDescription(
					'support-frontend failed to fetch one or more checkout nudge tests from DynamoDb',
				),
				actionsEnabled: shouldCreateAlarms,
				threshold: 1,
				evaluationPeriods: 1,
			...
			new GuAlarm(this, 'SalesTaxApiError', {
				app,
				alarmName: alarmName('support-frontend SalesTaxApiError'),
				alarmDescription: alarmDescription(
					'support-frontend failed to fetch data from the Sales Tax API',
				),
				actionsEnabled: shouldCreateAlarms,
				threshold: 1,
				evaluationPeriods: 1,
